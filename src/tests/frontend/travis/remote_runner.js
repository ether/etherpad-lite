...
const sauceTestWorker = async.queue(
	async ({ name, pfx, browser, version, platform }) => {
		const chromeOptions = new swdChrome.Options().addArguments(
			"use-fake-device-for-media-stream",
			"use-fake-ui-for-media-stream",
		);
		const edgeOptions = new swdEdge.Options().addArguments(
			"use-fake-device-for-media-stream",
			"use-fake-ui-for-media-stream",
		);
		...
		builder.getCapabilities().set("sauce:options", {
			...
			name: [process.env.GIT_HASH]
				.concat(process.env.SAUCE_NAME || [], name)
				.join(" - "),
			public: true,
		...
		const url = `https://saucelabs.com/jobs/${(
