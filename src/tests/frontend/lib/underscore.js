...
(function () {
	...
	_.invoke = (obj, method) => {
		...
		return _.map(obj, (value) =>
			(_.isFunction(method) ? method : value[method]).apply(value, args),
		);
	};
	...
	_.max = (obj, iterator, context) => {
		...
		if (!iterator && _.isEmpty(obj)) return Number.NEGATIVE_INFINITY;
		var result = { computed: Number.NEGATIVE_INFINITY };
		each(obj, (value, index, list) => {
	...
	_.min = (obj, iterator, context) => {
		...
		if (!iterator && _.isEmpty(obj)) return Number.POSITIVE_INFINITY;
		var result = { computed: Number.POSITIVE_INFINITY };
		each(obj, (value, index, list) => {
			var computed = iterator
				? iterator.call(context, value, index, list)
				: value;
	...
	var lookupIterator = (value) =>
		_.isFunction(value) ? value : (obj) => obj[value];

	// Sort the object's values by a criterion produced by an iterator.
	_.sortBy = (obj, value, context) => {
		var iterator = lookupIterator(value);
		return _.pluck(
			_.map(obj, (value, index, list) => ({
				value: value,
				index: index,
	...
	_.first =
		...
		_.take =
			(array, n, guard) =>
				n != null && !guard ? slice.call(array, 0, n) : array[0];
	...
	_.initial = (array, n, guard) =>
		slice.call(array, 0, array.length - (n == null || guard ? 1 : n));
	...
	_.rest =
		_.tail =
		_.drop =
			(array, n, guard) => slice.call(array, n == null || guard ? 1 : n);

	// Trim out all falsy values from an array.
	_.compact = (array) => _.filter(array, (value) => !!value);
	...
	_.union = () => _.uniq(concat.apply(ArrayProto, arguments));
	...
	_.intersection = (array) => {
		...
		return _.filter(_.uniq(array), (item) =>
			_.every(rest, (other) => _.indexOf(other, item) >= 0),
		);
	};
	...
	_.isArray =
		nativeIsArray || ((obj) => toString.call(obj) == "[object Array]");
	...
	_.isFinite = (obj) => _.isNumber(obj) && isFinite(obj);
