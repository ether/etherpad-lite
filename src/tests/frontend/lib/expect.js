((global, module) => {
		...
		if ($flags) {
			for (var i = 0, l = $flags.length; i < l; i++) {
				...
				if ("function" == typeof Assertion.prototype[name]) {
					...
					var old = this[name];
					this[name] = () => old.apply(this, arguments);
	...
	Assertion.prototype.throwError = Assertion.prototype.throwException =
		function (fn) {
	...
	Assertion.prototype.key = Assertion.prototype.keys = function ($keys) {
		...
		// Inclusion
		ok = every($keys, (key) => ~indexOf(actual, key));
	...
	function map(arr, mapper, that) {
		...
		for (var i = 0, n = arr.length; i < n; i++)
			if (i in arr) other[i] = mapper.call(that, arr[i], i, arr);
