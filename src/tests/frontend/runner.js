	...
	(async () => {
		...
		const customRunner = (runner) => {
			...
			const autoscroll = (newElement) => {
				...
				const scroll =
					...
					(() => {
						...
						const mrOffsetTopAbs =
							mr.getBoundingClientRect().top - mr.scrollTop;
			...
			runner.on("pass", (test) => {
				...
				killTimeout = setTimeout(() => {
					append(
						"FINISHED - [red]no test started since 5 minutes, tests stopped[clear]",
					);
				}, 60000 * 5);
			...
			runner.on("fail", (test, err) => {
				...
				test.err = err;
				append(
					`-> [red]FAILED[clear] : ${test.title} ${stringifyException(
						test.err,
					)}`,
				);
			});
		...
		window.testRunnerRequire = {
			define(...args) {
				...
				if (args.length !== 1)
					throw new Error("unexpected args passed to testRunnerRequire.define");
				const [origDefs] = args;
				const defs = {};
		...
		const appendToLog = (msg) => {
			...
			const scrolledToBottom =
				$log[0].scrollHeight <= $log[0].scrollTop + $log[0].clientHeight + 5;
			const $msg = $("<div>")
			...
			specs.map(async (spec) => {
				...
				try {
					await new Promise((resolve, reject) =>
						require(spec, (module) => {
							if (module == null)
								return reject(new Error(`failed to load module ${spec}`));
							resolve();
						}),
					);
				} catch (err) {
