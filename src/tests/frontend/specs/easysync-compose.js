...
const {
	randomMultiline,
	randomTestChangeset,
} = require("../easysync-helper.js");

describe("easysync-compose", () => {
	describe("compose", () => {
		const testCompose = (randomSeed) => {
			it(`testCompose#${randomSeed}`, async () => {
				...
				const change12 = Changeset.checkRep(
					Changeset.compose(change1, change2, p),
				);
				const change23 = Changeset.checkRep(
					Changeset.compose(change2, change3, p),
				);
				const change123 = Changeset.checkRep(
					Changeset.compose(change12, change3, p),
				);
				const change123a = Changeset.checkRep(
					Changeset.compose(change1, change23, p),
				);
	...
	describe("compose attributes", () => {
		it("simpleComposeAttributesTest", async () => {
			const p = new AttributePool();
			p.putAttrib(["bold", ""]);
