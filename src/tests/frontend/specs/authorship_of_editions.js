describe("author of pad edition", () => {
	...
	before(async () => {
		...
		const threeLines = [
			"regular line",
			"line with ordered list",
			"line with unordered list",
		].join("<br>");
		...
		await helper.waitForPromise(
			() =>
				getLine(LINE_WITH_UNORDERED_LIST).text() ===
					"line with unordered list" && helper.commits.length === 1,
		);
		...
		const $insertUnorderedListButton = helper.padChrome$(
		...
		const $insertOrderedListButton = helper.padChrome$(
		...
		const { Cookies } = helper.padChrome$.window.require(
	...
	it("regular line", async () => {
		await changeLineAndCheckOnlyThatChangeIsFromThisAuthor(REGULAR_LINE, "x");
	});

	it("line with ordered list", async () => {
		await changeLineAndCheckOnlyThatChangeIsFromThisAuthor(
			LINE_WITH_ORDERED_LIST,
			"y",
		);
	});

	it("line with unordered list", async () => {
		await changeLineAndCheckOnlyThatChangeIsFromThisAuthor(
	...
	const getAuthorFromClassList = (classes) =>
		classes.find((cls) => cls.startsWith("author"));

	const changeLineAndCheckOnlyThatChangeIsFromThisAuthor = async (
		lineNumber,
		textChange,
	) => {
		// get original author class
		const classes = getLine(lineNumber)
			.find("span")
			.first()
			.attr("class")
			.split(" ");
		...
		await helper.waitForPromise(() => {
			const authorsOfLine = getLine(lineNumber)
				.find("span")
				.map(function () {
					return getAuthorFromClassList($(this).attr("class").split(" "));
				})
				.get();
			otherAuthorsOfLine = authorsOfLine.filter(
				(author) => author !== originalAuthor,
			);
