describe("importexport.js", () => {
	const testCases = [
		{
			...
			wantExportHtmlBody: ["imported text<br>", "newline<br>"].join(""),
			wantExportText: ["imported text\n", "newline\n"].join(""),
		},
		{
			name: "HTML with newlines",
			inputHtmlBody: ["htmltext<br>", "newline"].join(""),
			wantPadLines: [
				'<span class="">htmltext</span>',
				'<span class="">newline</span>',
				"<br>",
			],
			wantExportHtmlBody: ["htmltext<br>", "newline<br>", "<br>"].join(""),
	...
	beforeEach(async () => {
		...
		const button = helper.padChrome$(
			'button[data-l10n-id="pad.toolbar.import_export.title"]',
		);
	...
	for (const tc of testCases) {
		describe(tc.name, () => {
			it("import", async () => {
				...
				const contents =
					ext === "html"
						? docToHtml(htmlBodyToDoc(tc.inputHtmlBody))
						: tc.inputText;
				// DataTransfer is used as a hacky way to get a mutable FileList. For details, see:
				// https://stackoverflow.com/q/47119426
				const dt = new DataTransfer();
				dt.items.add(
					new File([contents], `file.${ext}`, { type: "text/plain" }),
				);
				...
				try {
					...
					const g = helper
						.linesDiv()
						.map((div) => formatLine(div.html()))
						.join("\n");
					const w = tc.wantPadLines.map(formatLine).join("\n");
					throw new Error(
						`Import failed. Got pad lines:\n${g}\nWant pad lines:\n${w}`,
					);
				}
			});

			it("export to HTML", async () => {
