describe("the test helper", () => {
	describe("the newPad method", () => {
		...
		xit("gives me 3 jquery instances of chrome, outer and inner", async function () {
			...
			expect(helper.padChrome$.window.document).to.be(
		...
		xit("clears cookies", async () => {
			...
			const getPadcookie = () =>
				helper.padChrome$.window.require(
		...
		it("sets pad prefs cookie", async () => {
	...
	describe("the waitFor method", () => {
		...
		it("rejects if the predicate throws", async () => {
			let err;
			await helper
				.waitFor(() => {
		...
		describe("returns a deferred object", () => {
			it("it calls done after success", (done) => {
				helper
					.waitFor(() => true)
					.done(() => {
						done();
			...
			it("calls fail after failure", (done) => {
				helper
					.waitFor(() => false, 0)
					.fail(() => {
						done();
			...
			xit("throws if you don't listen for fails", (done) => {
		...
		describe("checks first then sleeps", () => {
			it("resolves quickly if the predicate is immediately true", async () => {
				const before = Date.now();
				await helper.waitFor(() => true, 1000, 900);
				expect(Date.now() - before).to.be.lessThan(800);
			});

			xit("polls exactly once if timeout < interval", async () => {
				let calls = 0;
				await helper
					.waitFor(
						() => {
			...
			it("resolves if condition is immediately true even if timeout is 0", async () => {
	...
	describe("the waitForPromise method", () => {
	...
	describe("the selectLines method", () => {
		...
		before(async () => {
			...
			$firstLine.sendkeys(
				"{selectall}some{enter}short{enter}lines{enter}to test{enter}{enter}",
			);
		...
		xit(
			"changes editor selection to be between startOffset of $startLine " +
				"and endOffset of $endLine",
			(done) => {
				 ...
				 */
				expect(
					cleanText(selection.toString().replace(/(\r\n|\n|\r)/gm, "")),
				).to.be("ort lines to t");

				done();
			},
		);

		it("ends selection at beginning of $endLine when it is an empty line", (done) => {
			const inner$ = helper.padInner$;
			...
			expect(
				cleanText(selection.toString().replace(/(\r\n|\n|\r)/gm, "")),
			).to.be("ort lines to test");

			done();
		});

		it("ends selection at beginning of $endLine when its offset is zero", async () => {
			const inner$ = helper.padInner$;
		...
		it("selects full line when offset is longer than line content", (done) => {
			const inner$ = helper.padInner$;
		...
		it(
			"selects all text between beginning of $startLine and end of $endLine " +
				"when no offset is provided",
			async () => {
				const inner$ = helper.padInner$;
	...
	describe("helper", () => {
		...
		it(".textLines() returns the text of the pad as strings", async () => {
		...
		it(".linesDiv() returns the text of the pad as div elements", async () => {
		...
		xit(".edit() defaults to send an edit to the first line", async () => {
			const firstLine = helper.textLines()[0];
			await helper.edit("line");
			expect(helper.textLines()[0]).to.be(`line${firstLine}`);
		});

		xit(".edit() to the line specified with parameter lineNo", async () => {
			const firstLine = helper.textLines()[0];
		...
		xit(".edit() supports sendkeys syntax ({selectall},{del},{enter})", async () => {
