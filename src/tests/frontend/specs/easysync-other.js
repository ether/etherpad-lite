...
describe("easysync-other", () => {
	describe("filter attribute numbers", () => {
		const testFilterAttribNumbers = (testId, cs, filter, correctOutput) => {
			it(`testFilterAttribNumbers#${testId}`, async () => {
				const str = Changeset.filterAttribNumbers(cs, filter);
				expect(str).to.equal(correctOutput);
			});
		};

		testFilterAttribNumbers(
			1,
			"*0*1+1+2+3*1+4*2+5*0*2*1*b*c+6",
			(n) => n % 2 === 0,
	...
	describe("make attribs string", () => {
		const testMakeAttribsString = (
			testId,
			pool,
			opcode,
			attribs,
			...
			it(`testMakeAttribsString#${testId}`, async () => {
				const p = poolOrArray(pool);
				padutils.warnDeprecated.disabledForTestingOnly = true;
				try {
					expect(Changeset.makeAttribsString(opcode, attribs, p)).to.equal(
						correctString,
					);
				} finally {
	...
	describe("other", () => {
		it("testMoveOpsToNewPool", async () => {
			...
			expect(
				Changeset.moveOpsToNewPool("Z:1>2*1+1*0+1$ab", pool1, pool2),
			).to.equal("Z:1>2*0+1*1+1$ab");
			expect(Changeset.moveOpsToNewPool("*1+1*0+1", pool1, pool2)).to.equal(
				"*0+1*1+1",
			);
		});

		it("testMakeSplice", async () => {
			const t = "a\nb\nc\n";
		...
		it("makeSplice at the end", async () => {
			...
			const ins = "456";
			expect(
				Changeset.applyToText(
					Changeset.makeSplice(orig, orig.length, 0, ins),
					orig,
				),
			).to.equal(`${orig}${ins}`);
		});

		it("testToSplices", async () => {
			const cs = Changeset.checkRep(
				"Z:z>9*0=1=4-3+9=1|1-4-4+1*0+a$123456789abcdefghijk",
			);
			const correctSplices = [
				[5, 8, "123456789"],
				[9, 17, "abcdefghijk"],
			];
			expect(Changeset.exportedForTestingOnly.toSplices(cs)).to.eql(
				correctSplices,
			);
		});

		it("opAttributeValue", async () => {
		...
		describe("applyToAttribution", () => {
			const runApplyToAttributionTest = (
				testId,
				attribs,
				cs,
				inAttr,
				...
				it(`applyToAttribution#${testId}`, async () => {
		...
		describe("split/join attribution lines", () => {
			const testSplitJoinAttributionLines = (randomSeed) => {
				...
				it(`testSplitJoinAttributionLines#${randomSeed}`, async () => {
					const doc = `${randomMultiline(10, 20)}\n`;

					const theJoined = stringToOps(doc);
					const theSplit = doc.match(/[^\n]*\n/g).map(stringToOps);

					expect(Changeset.splitAttributionLines(theJoined, doc)).to.eql(
						theSplit,
					);
