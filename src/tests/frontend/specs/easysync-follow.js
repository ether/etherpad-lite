...
describe("easysync-follow", () => {
	describe("follow & compose", () => {
		const testFollow = (randomSeed) => {
			it(`testFollow#${randomSeed}`, async () => {
	...
	describe("followAttributes & composeAttributes", () => {
		const p = new AttributePool();
		p.putAttrib(["x", ""]);
		p.putAttrib(["x", "abc"]);
		p.putAttrib(["x", "def"]);
		...
		const testFollow = (a, b, afb, bfa, merge) => {
			it(`manual #${++n}`, async () => {
				expect(
					Changeset.exportedForTestingOnly.followAttributes(a, b, p),
				).to.equal(afb);
				expect(
					Changeset.exportedForTestingOnly.followAttributes(b, a, p),
				).to.equal(bfa);
	...
	describe("chracterRangeFollow", () => {
		const testCharacterRangeFollow = (
			...
			it(`testCharacterRangeFollow#${testId}`, async () => {
