...
describe("easysync-inverseRandom", () => {
	describe("inverse random", () => {
		const testInverseRandom = (randomSeed) => {
			it(`testInverseRandom#${randomSeed}`, async () => {
				const p = poolOrArray([
					"apple,",
					"apple,true",
					"banana,",
				...
				const alines = Changeset.splitAttributionLines(
					Changeset.makeAttribution(startText),
					startText,
				);
				const lines = startText
					.slice(0, -1)
					.split("\n")
					.map((s) => `${s}\n`);
	...
	describe("inverse", () => {
		const testInverse = (testId, cs, lines, alines, pool, correctOutput) => {
			it(`testInverse#${testId}`, async () => {
				pool = poolOrArray(pool);
				const str = Changeset.inverse(
