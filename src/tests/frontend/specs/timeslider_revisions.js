describe("timeslider", () => {
	...
	xit("changes the url when clicking on the timeslider", async () => {
		...
		await helper.waitForPromise(
			() =>
				$("#iframe-container iframe")[0].contentWindow.location.hash !== oldUrl,
			6000,
		);
	});
	...
	it("jumps to a revision given in the url", async function () {
		...
		await helper.waitForPromise(() => {
			...
			const colorOkay =
				inner$("span").first().attr("class").indexOf("author-") === 0;
		...
		await helper.waitForPromise(() => {
			...
			return (
				timeslider$ && timeslider$("#innerdocbody").text().length === oldLength
			);
		}, 10000);
	});
