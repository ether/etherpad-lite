...
describe("easysync-assembler", () => {
	it("opAssembler", async () => {
	...
	it("smartOpAssembler ignore additional pure keeps (no attributes)", async () => {
	...
	it("smartOpAssembler merge consecutive + ops without multiline", async () => {
	...
	it("smartOpAssembler merge consecutive + ops with multiline", async () => {
	...
	it("smartOpAssembler ignore + ops with ops.chars === 0", async () => {
	...
	it("smartOpAssembler append + op with text", async () => {
		const assem = Changeset.smartOpAssembler();
	...
	it("smartOpAssembler append + op with multiline text", async () => {
		const assem = Changeset.smartOpAssembler();
		const pool = poolOrArray([
			"attr1,1",
			"attr2,2",
	...
	it("smartOpAssembler clear should empty internal assemblers", async () => {
		const x = "-c*3*4+6|3=az*asdf0*1*2*3+1=1-1+1*0+1=1-1+1|c=c-1";
	...
	describe("append atext to assembler", () => {
		const testAppendATextToAssembler = (testId, atext, correctOps) => {
			it(`testAppendATextToAssembler#${testId}`, async () => {
				const assem = Changeset.smartOpAssembler();
