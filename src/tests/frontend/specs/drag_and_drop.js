...
describe("drag and drop", () => {
	...
	context(
		"when user drags part of one line and drops it far form its original place",
		() => {
			before(async () => {
				...
				await helper.waitForPromise(() => {
					...
					const sourceWasMovedToTarget =
						$targetLine.text() === "Target line [line 1]";
					return sourceWasMovedToTarget;
				});
			});

			context("and user triggers UNDO", () => {
				before(async () => {
				...
				it("moves text back to its original place", async () => {
	...
	context("when user drags some lines far form its original place", () => {
		before(async () => {
