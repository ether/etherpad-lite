...
describe("AttributeMap", () => {
	...
	describe("set", () => {
		...
		it("reuses attributes in the pool", async () => {
		...
		it("inserts new attributes into the pool", async () => {
			const m = new AttributeMap(pool);
			expect(getPoolSize()).to.equal(attribs.length);
			m.set("k", "v");
		...
		describe("coerces key and value to string", () => {
			...
			for (const [desc, input, want] of testCases) {
				describe(desc, () => {
					it("key is coerced to string", async () => {
						const m = new AttributeMap(pool);
						m.set(input, "value");
						expect(m.get(want)).to.equal("value");
					});

					it("value is coerced to string", async () => {
						const m = new AttributeMap(pool);
						m.set("key", input);
	...
	describe("toString", () => {
		...
		it("returns all entries", async () => {
			...
			expect(m.toString()).to.equal(
				attributes.encodeAttribString([attribs.length]),
			);
			m.set(...attribs[2]);
			expect(m.toString()).to.equal(
				attributes.attribsToString(
					attributes.sort([attribs[2], [attribs[1][0], "new value"]]),
					pool,
				),
			);
		});
	...
	for (const funcName of ["update", "updateFromString"]) {
		const callUpdateFn = (m, ...args) => {
			if (funcName === "updateFromString") {
				args[0] = attributes.attribsToString(
					attributes.sort([...args[0]]),
					pool,
				);
			}
		...
		describe(funcName, () => {
			it("works", async () => {
				...
				const wantStr = attributes.attribsToString(
					attributes.sort([...attribs]),
					pool,
				);
			...
			describe("emptyValueIsDelete=false inserts empty values", () => {
				for (const emptyVal of ["", null, undefined]) {
					it(
						emptyVal == null ? String(emptyVal) : JSON.stringify(emptyVal),
						async () => {
							const m = new AttributeMap(pool);
							m.set("k", "v");
							...
							expect(m.size).to.equal(1);
							expect(m.toString()).to.equal(
								attributes.attribsToString([["k", ""]], pool),
							);
						},
					);
				}
			...
			describe("emptyValueIsDelete=true deletes entries", () => {
				for (const emptyVal of ["", null, undefined]) {
					it(
						emptyVal == null ? String(emptyVal) : JSON.stringify(emptyVal),
						async () => {
