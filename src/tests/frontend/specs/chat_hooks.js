describe("chat hooks", () => {
	...
	const loadPad = async (opts = {}) => {
		...
		ChatMessage = helper.padChrome$.window.require(
			"ep_etherpad-lite/static/js/ChatMessage",
		);
		({ hooks } = helper.padChrome$.window.require(
	...
	describe("chatNewMessage", () => {
		...
		it("message.text is not processed", async () => {
			...
			await Promise.all([
				checkHook("chatNewMessage", (context) => {
					...
					rendered = context.rendered =
						helper.padChrome$.document.createElement("p");
	...
	describe("chatSendMessage", () => {
		...
		it("message metadata propagates end-to-end", async function () {
			...
			await Promise.all([
				...
				checkHook("chatNewMessage", ({ message: { customMetadata } }) => {
					expect(JSON.stringify(customMetadata)).to.equal(
		...
		it("message metadata is saved in the database", async function () {
			...
			expect(message).to.be.a(ChatMessage);
			expect(JSON.stringify(message.customMetadata)).to.equal(
				JSON.stringify(metadata),
			);
		});
	});
