describe("Automatic pad reload on Force Reconnect message", () => {
	...
	beforeEach(async () => {
		...
		const $otherIframeWithSamePad = $(
			`<iframe src="/p/${padId}" style="height: 1px;"></iframe>`,
		);
	...
	it("displays a count down timer to automatically reconnect", async () => {
		const $errorMessageModal = helper.padChrome$("#connectivity .userdup");
	...
	context("and user clicks on Cancel", () => {
		beforeEach(async () => {
			const $errorMessageModal = helper.padChrome$("#connectivity .userdup");
		...
		it("does not show Cancel button nor timer anymore", async () => {
			const $errorMessageModal = helper.padChrome$("#connectivity .userdup");
			const $countDownTimer = $errorMessageModal.find(".reconnecttimer");
	...
	context("and user does not click on Cancel until timer expires", () => {
