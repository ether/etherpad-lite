...
describe('Admin page', function () {
  ...
  beforeEach(async function () {
    ...
    await helper.waitForPromise(
        () => helper.admin$ && helper.admin$('.menu').find('li').length >= 3, 5000);
  });

  it('Shows Plugin Manager Link', async function () {
    helper.waitForPromise(
        () => helper.admin$('a[data-l10n-id="admin_plugins"]').is(':visible'), 5000);
  });
  it('Shows Troubleshooting Info Link', async function () {
    helper.waitForPromise(
        () => helper.admin$('a[data-l10n-id="admin_plugins_info"]').is(':visible'), 5000);
  });
  it('Shows Settings Link', async function () {
    helper.waitForPromise(
        () => helper.admin$('a[data-l10n-id="admin_settings"]').is(':visible'), 5000);
  });
});
