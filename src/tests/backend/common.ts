...
export const init = async () => {
	...
	if (!logLevel.isLessThanOrEqualTo(log4js.levels.DEBUG)) {
		logger.warn(
			"Disabling non-test logging for the duration of the test. " +
...
export const waitForSocketEvent = async (socket: any, event: string) => {
	...
	try {
		...
		const errorEventP = Promise.race(
			errorEvents.map(
				(event) =>
...
export const connect = async (res: any = null) => {
	...
	const reqCookieHdr = Object.entries(resCookies)
		.map(
			// @ts-ignore
			([name, cookie]) => `${name}=${encodeURIComponent(cookie.value)}`,
		)
		.join("; ");
 ...
 */
export const sendUserChanges = async (socket: any, data: any) =>
...
export const randomString = (
	len = 10,
	charset = `${alphabet}${alphabet.toUpperCase()}0123456789`,
): string => {
	let ret = "";
