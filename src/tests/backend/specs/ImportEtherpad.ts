...
describe(__filename, () => {
	...
	it("unknown db records are ignored", async () => {
	...
	it("changes are all or nothing", async () => {
		...
		assert.rejects(
			importEtherpad.setPadRaw(padId, JSON.stringify(data)),
			/unexpected pad ID/,
		);
	...
	describe("author pad IDs", () => {
		let existingAuthorId: string;
		let newAuthorId: string;

		beforeEach(async () => {
			existingAuthorId = (await authorManager.createAuthor("existing"))
				.authorID;
			assert(await authorManager.doesAuthorExist(existingAuthorId));
			assert.deepEqual(
				(await authorManager.listPadsOfAuthor(existingAuthorId)).padIDs,
				[],
			);
		...
		it("author does not yet exist", async () => {
			await importEtherpad.setPadRaw(
				padId,
				JSON.stringify(makeExport(newAuthorId)),
			);
			assert(await authorManager.doesAuthorExist(newAuthorId));
			...
			assert.deepEqual(
				(await authorManager.listPadsOfAuthor(newAuthorId)).padIDs,
				[padId],
			);
		});

		it("author already exists, no pads", async () => {
		...
		it("author already exists, on different pad", async () => {
		...
		it("author already exists, on same pad", async () => {
	...
	describe("enforces consistent pad ID", () => {
		it("pad record has different pad ID", async () => {
		...
		it("globalAuthor record has different pad ID", async () => {
			const authorId = makeAuthorId();
		...
		it("pad rev record has different pad ID", async () => {
			const data: MapArrayType<any> = makeExport(makeAuthorId());
			data["pad:differentPadId:revs:0"] = data["pad:testing:revs:0"];
			delete data["pad:testing:revs:0"];
			assert.rejects(
	...
	describe("order of records does not matter", () => {
		for (const perm of [
			[0, 1, 2],
			[0, 2, 1],
			[1, 0, 2],
			...
			it(JSON.stringify(perm), async () => {
				const authorId = makeAuthorId();
				const records = Object.entries(makeExport(authorId));
				assert.equal(records.length, 3);
				await importEtherpad.setPadRaw(
					padId,
					JSON.stringify(Object.fromEntries(perm.map((i) => records[i]))),
				);
				assert.deepEqual(
					(await authorManager.listPadsOfAuthor(authorId)).padIDs,
					[padId],
				);
	...
	describe("exportEtherpadAdditionalContent", () => {
		let hookBackup: Function;

		before(async () => {
			hookBackup = plugins.hooks.exportEtherpadAdditionalContent || [];
			plugins.hooks.exportEtherpadAdditionalContent = [
				{ hook_fn: () => ["custom"] },
			];
		});
		...
		it("imports from custom prefix", async () => {
			await importEtherpad.setPadRaw(
				padId,
				JSON.stringify({
					...makeExport(makeAuthorId()),
		...
		it("rejects records for pad with similar ID", async () => {
