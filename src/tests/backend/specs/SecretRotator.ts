...
const SecretRotator =
	require("../../../node/security/SecretRotator").SecretRotator;
...
const lcm: Function = (...args: number[]) =>
	args.length === 1
		? args[0]
		: args.length === 2
			? Math.abs(args[0] * args[1]) / gcd(...args)
...
class FakeClock {
	...
	_next() {
		return Math.min(...[...this.timeouts.values()].map((x) => x.when));
	}
	async setNow(t: number) {
		logger.debug(`setting fake time to ${t}`);
		strict(t >= this._now);
		strict(t < Number.POSITIVE_INFINITY);
		let n;
	...
	async advance(t: number) {
		await this.setNow(this._now + t);
	}
	async advanceToNext() {
		const n = this._next();
		if (n < this._now) await this._fire();
		else if (n < Number.POSITIVE_INFINITY) await this.setNow(n);
	}
	async _fire() {
...
describe(__filename, () => {
	...
	const hkdf = async (secret: string, salt: string, tN: number) =>
		Buffer.from(
			await crypto.hkdf("sha256", secret, salt, `${tN}`, 32),
		).toString("hex");

	const newRotator = (s: string | null = null) =>
		new SecretRotator(dbPrefix, interval, lifetime, s);

	const setFakeClock = (
		sr: {
			_t: {
				now: () => number;
				setTimeout: (fn: Function, wait?: number) => number;
				clearTimeout: (id: number) => void;
			};
		},
		fc: FakeClock | null = null,
	) => {
	...
	afterEach(async () => {
		...
		sr = null;
		await Promise.all(
			(await db.findKeys(`${dbPrefix}:*`, null)).map(
				async (dbKey: string) => await db.remove(dbKey),
			),
		);
	});

	describe("constructor", () => {
		it("creates empty secrets array", async () => {
			sr = newRotator();
			strict.deepEqual(sr.secrets, []);
		});

		for (const invalidChar of "*:%") {
			it(`rejects database prefixes containing ${invalidChar}`, async () => {
	...
	describe("start", () => {
		it("does not replace secrets array", async () => {
			sr = newRotator();
			setFakeClock(sr);
			const { secrets } = sr;
			await sr.start();
		...
		it("publishes params", async () => {
			...
			strict.deepEqual(
				sr.secrets,
				await Promise.all(
					[0, -interval, interval].map(
						async (tN) => await hkdf(secret, salt, tN),
					),
				),
			);
		});

		it("reuses matching publication if unexpired", async () => {
			sr = newRotator();
			const fc = setFakeClock(sr);
			await sr.start();
			const { secrets } = sr;
		...
		it("deletes expired publications", async () => {
			sr = newRotator();
			const fc = setFakeClock(sr);
			await sr.start();
		...
		it("keeps expired publications until interval past expiration", async () => {
			sr = newRotator();
			const fc = setFakeClock(sr);
			await sr.start();
		...
		describe(`schedules update at next interval (= ${interval})`, () => {
			const testCases = [
				{ now: 0, want: interval },
				{ now: 1, want: interval },
			...
			it("multiple active params with different intervals", async () => {
				const intervals = [400, 600, 1000];
				const lcmi = lcm(...intervals);
				const wants: Set<number> = new Set();
				for (const i of intervals)
					for (let t = i; t <= lcmi; t += i) wants.add(t);
	...
	describe("legacy secret", () => {
		it("ends at now if there are no previously published secrets", async () => {
			...
			const ids = await db.findKeys(`${dbPrefix}:*`, null);
			const params = (
				await Promise.all(ids.map(async (id: string) => await db.get(id)))
		...
		it("ends at the start of the oldest previously published secret", async () => {
			sr = newRotator();
			const fc = setFakeClock(sr);
			await fc.setNow(1);
			strict(mod(fc.now, interval) !== 0);
		...
		it("multiple instances with different legacy secrets", async () => {
		...
		it("multiple instances with the same legacy secret", async () => {
			sr = newRotator("legacy");
			const fc = setFakeClock(sr);
			await sr.start();
			sr.stop();
		...
		it("legacy secret is included for interval after expiration", async () => {
			sr = newRotator();
			const fc = setFakeClock(sr);
			await sr.start();
			sr.stop();
		...
		it("legacy secret is not included if the oldest secret is old enough", async () => {
			sr = newRotator();
			const fc = setFakeClock(sr);
			await sr.start();
			sr.stop();
		...
		it("dead secrets still affect legacy secret end time", async () => {
			sr = newRotator();
			const fc = setFakeClock(sr);
			await sr.start();
	...
	describe("rotation", () => {
		it("no rotation before start of interval", async () => {
			sr = newRotator();
			const fc = setFakeClock(sr);
		...
		it("future secret becomes current, new future is generated", async () => {
			sr = newRotator();
			const fc = setFakeClock(sr);
			await sr.start();
			const secrets = new Set(sr.secrets);
		...
		it("expired publications are deleted", async () => {
		...
		it("old secrets are eventually removed", async () => {
			sr = newRotator();
			const fc = setFakeClock(sr);
			await sr.start();
			const [, s0] = sr.secrets;
	...
	describe("clock skew", () => {
		it("out of sync works if in adjacent interval", async () => {
