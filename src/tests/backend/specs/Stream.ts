...
class DemoIterable {
	...
	completed() {
		return this.errs.length > 0 || this.rets.length > 0;
	}
...
describe(__filename, () => {
	describe("basic behavior", () => {
		...
		it("takes an iterator", async () => {
			strict.deepEqual(
				[...new Stream([0, 1, 2][Symbol.iterator]())],
				[0, 1, 2],
			);
		});

		it("supports empty iterators", async () => {
		...
		it("does not start until needed", async () => {
			let lastYield = null;
			new Stream(
				(function* () {
					yield (lastYield = 0);
	...
	describe("batch", () => {
		...
		it("fewer than batch size", async () => {
			let lastYield = null;
		...
		it("multiple batches, last batch is not full", async () => {
			let lastYield = null;
			const values = (function* () {
		...
		it("batched Promise rejections are suppressed while iterating", async () => {
			let lastYield = null;
			const err = new Error("injected");
			const values = (function* () {
		...
		it("batched Promise rejections are unsuppressed when iteration completes", async () => {
			let lastYield = null;
			const err = new Error("injected");
			const values = (function* () {
				lastYield = "promise of 0";
	...
	describe("buffer", () => {
		...
		it("fewer than buffer size", async () => {
			let lastYield = null;
			const values = (function* () {
				for (let i = 0; i < 5; i++) yield (lastYield = i);
			})();
		...
		it("more than buffer size", async () => {
			let lastYield = null;
			const values = (function* () {
				for (let i = 0; i < 10; i++) yield (lastYield = i);
			})();
		...
		it("buffered Promise rejections are suppressed while iterating", async () => {
			let lastYield = null;
			const err = new Error("injected");
			const values = (function* () {
				lastYield = "promise of 0";
		...
		it("buffered Promise rejections are unsuppressed when iteration completes", async () => {
			let lastYield = null;
			const err = new Error("injected");
			const values = (function* () {
				lastYield = "promise of 0";
