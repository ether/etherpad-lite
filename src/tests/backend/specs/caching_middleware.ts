...
describe(__filename, () => {
	...
	for (const minify of [false, true]) {
		context(`when minify is ${minify}`, () => {
			before(async () => {
				settings.minify = minify;
			});

			describe("gets packages uncompressed without Accept-Encoding gzip", () => {
			...
			describe("gets packages compressed with Accept-Encoding gzip", () => {
				for (const resource of packages) {
					it(resource, async () => {
						await agent
							.get(resource)
			...
			it("does not cache content-encoding headers", async () => {
				await agent
					.get(packages[0])
					.set("Accept-Encoding", fantasyEncoding)
					.expect(200)
