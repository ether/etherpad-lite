 ...
 */
const ulSpaceHtml =
	'<!doctype html><html><body><ul class="bullet"> <li>one</li></ul></body></html>';
...
const expectedSpaceHtml =
	'<!doctype html><html><body><ul class="bullet"><li>one</ul></body></html>';
	...
	describe("copyPadWithoutHistory", () => {
		...
		it("creates a new pad with the same content as the source pad", async () => {
			...
			res = await agent
				...
				.expect(200);
			const receivedHtml = res.body.data.html
				.replace("<br><br></body>", "</body>")
				.toLowerCase();
			assert.equal(receivedHtml, expectedHtml);
		});

		it("copying to a non-existent group throws an error", async () => {
			const padWithNonExistentGroup = `notExistentGroup$${newPad}`;
			const res = await agent
				.get(
					`${endPoint("copyPadWithoutHistory")}` +
		...
		describe("copying to an existing pad", () => {
			beforeEach(async () => {
				await createNewPadWithHtml(newPad, ulHtml);
			});

			it("force=false fails", async () => {
				const res = await agent
					.get(
						`${endPoint("copyPadWithoutHistory")}` +
							`?sourceID=${sourcePadId}` +
		...
		it("source and destination attribute pools are independent", async () => {
			...
			const getHtml = async (padId: string) => {
				const res = await agent
					.get(`${endPoint("getHTML")}?padID=${padId}`)
					.set("Authorization", await common.generateJWTToken())
					.expect(200)
					.expect("Content-Type", /json/);
				assert.equal(res.body.code, 0);
			...
			let [srcHtml, dstHtml] = await Promise.all([
				getHtml(sourcePadId),
				getHtml(newPad),
			]);
