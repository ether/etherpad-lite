	...
	describe("Imports and Exports", () => {
		...
		it("exports HTML for this Etherpad file", async function () {
			...
			await agent
				...
				.expect(
					/<ul class="bullet"><li><ul class="bullet"><li>hello<\/ul><\/li><\/ul>/,
				);
		});
		...
		describe("malformed .etherpad files are rejected", () => {
			...
			const importEtherpad = (records: any) =>
				agent
					.post(`/p/${testPadId}/import`)
			...
			it("changeset refers to non-existent attrib", async () => {
				const records: MapArrayType<any> = makeGoodExport();
			...
			it("pad atext does not match", async () => {
				const records = makeGoodExport();
				records["pad:testing"].atext.attribs =
					`*0${records["pad:testing"].atext.attribs}`;
				importEtherpad(records).expect(500);
			});

			it("missing chat message", async () => {
				const records: MapArrayType<any> = makeGoodExport();
		...
		describe("revisions are supported in txt and html export", () => {
			const makeGoodExport = () => ({
				"pad:testing": {
					atext: {
						text: "oofoo\n",
			...
			const importEtherpad = (records: MapArrayType<any>) =>
				agent
					.post(`/p/${testPadId}/import`)
					.attach("file", Buffer.from(JSON.stringify(records), "utf8"), {
						filename: "/test.etherpad",
			...
			it("txt request rev 1", async () => {
				await agent
					.get(`/p/${testPadId}/1/export/txt`)
					.set("authorization", await common.generateJWTToken())
					.expect(200)
					.buffer(true)
					.parse(superagent.parse.text)
			...
			it("txt request rev 2", async () => {
				await agent
					.get(`/p/${testPadId}/2/export/txt`)
					.set("authorization", await common.generateJWTToken())
					.expect(200)
					.buffer(true)
					.parse(superagent.parse.text)
			...
			it("txt request rev 1test returns rev 1", async () => {
				await agent
					.get(`/p/${testPadId}/1test/export/txt`)
					.set("authorization", await common.generateJWTToken())
					.expect(200)
					.buffer(true)
					.parse(superagent.parse.text)
			...
			it("txt request rev test1 is 403", async () => {
				await agent
					.get(`/p/${testPadId}/test1/export/txt`)
					.set("authorization", await common.generateJWTToken())
					.expect(500)
					.buffer(true)
					.parse(superagent.parse.text)
			...
			it("txt request rev 5 returns head rev", async () => {
				await agent
					.get(`/p/${testPadId}/5/export/txt`)
					.set("authorization", await common.generateJWTToken())
					.expect(200)
					.buffer(true)
					.parse(superagent.parse.text)
			...
			it("html request rev 1", async () => {
				await agent
					.get(`/p/${testPadId}/1/export/html`)
					.set("authorization", await common.generateJWTToken())
					.expect(200)
					.buffer(true)
					.parse(superagent.parse.text)
			...
			it("html request rev 2", async () => {
				await agent
					.get(`/p/${testPadId}/2/export/html`)
					.set("authorization", await common.generateJWTToken())
					.expect(200)
					.buffer(true)
					.parse(superagent.parse.text)
			...
			it("html request rev 1test returns rev 1", async () => {
				await agent
					.get(`/p/${testPadId}/1test/export/html`)
					.set("authorization", await common.generateJWTToken())
					.expect(200)
					.buffer(true)
					.parse(superagent.parse.text)
			...
			it("html request rev test1 results in 500 response", async () => {
				await agent
					.get(`/p/${testPadId}/test1/export/html`)
					.set("authorization", await common.generateJWTToken())
					.expect(500)
					.buffer(true)
					.parse(superagent.parse.text)
			...
			it("html request rev 5 returns head rev", async () => {
				await agent
					.get(`/p/${testPadId}/5/export/html`)
					.set("authorization", await common.generateJWTToken())
					.expect(200)
					.buffer(true)
					.parse(superagent.parse.text)
		...
		describe("Import authorization checks", function () {
			...
			it("!authn !exist -> create", async () => {
				await agent
					.post(`/p/${testPadIdEnc}/import`)
					.set("authorization", await common.generateJWTToken())
					.attach("file", padText, {
						filename: "/test.txt",
						contentType: "text/plain",
			...
			it("authn anonymous !exist -> fail", async () => {
			...
			it("authn user modify !exist -> fail", async () => {
				settings.requireAuthentication = true;
				authorize = () => "modify";
				await agent
					.post(`/p/${testPadIdEnc}/import`)
			...
			it("authn user readonly !exist -> fail", async () => {
				settings.requireAuthentication = true;
				authorize = () => "readOnly";
				await agent
					.post(`/p/${testPadIdEnc}/import`)
			...
			it("authn user create exist -> replace", async () => {
				settings.requireAuthentication = true;
				const pad = await createTestPad("before import\n");
				await agent
					.post(`/p/${testPadIdEnc}/import`)
			...
			it("authn user modify exist -> replace", async () => {
				settings.requireAuthentication = true;
				authorize = () => "modify";
				const pad = await createTestPad("before import\n");
				await agent
