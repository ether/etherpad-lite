  ...
  */
	...
	describe("Chat functionality", () => {
		it("creates a new Pad", async () => {
			await agent
				.get(`${endPoint("createPad")}?padID=${padID}`)
		...
		it("Creates an author with a name set", async () => {
			await agent
		...
		it("Gets the head of chat before the first chat msg", async () => {
			await agent
				.get(`${endPoint("getChatHead")}?padID=${padID}`)
				.set("authorization", await generateJWTToken())
				.expect((res: any) => {
					if (res.body.data.chatHead !== -1)
		...
		it("Adds a chat message to the pad", async () => {
			await agent
				.get(
					`${endPoint("appendChatMessage")}?padID=${padID}&text=blalblalbha` +
		...
		it("Gets the head of chat", async () => {
			await agent
				.get(`${endPoint("getChatHead")}?padID=${padID}`)
				.set("authorization", await generateJWTToken())
				.expect((res: any) => {
		...
		it("Gets Chat History of a Pad", async () => {
			await agent
				.get(`${endPoint("getChatHistory")}?padID=${padID}`)
				.set("authorization", await generateJWTToken())
				.expect("Content-Type", /json/)
				.expect(200)
				.expect((res: any) => {
					...
					assert.equal(
						...
						1,
						"Chat History Length is wrong",
					);
					assert.equal(
						res.body.data.messages[0].text,
						"blalblalbha",
						"Chat text does not match",
					);
					assert.equal(
						res.body.data.messages[0].userId,
						authorID,
						"Message author does not match",
					);
					assert.equal(
						res.body.data.messages[0].time,
						timestamp.toString(),
