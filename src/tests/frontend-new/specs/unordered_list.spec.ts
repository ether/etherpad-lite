...
test.describe("unordered_list.js", function () {
	test.describe("assign unordered list", function () {
		test("insert unordered list text then removes by outdent", async function ({
			page,
		}) {
	...
	test.describe("unassign unordered list", function () {
		...
		test("insert unordered list text then remove by clicking list again", async function ({
			page,
		}) {
			const padBody = await getPadBody(page);
			const originalText = await padBody.locator("div").first().textContent();
	...
	test.describe("keep unordered list on enter key", function () {
		test("Keeps the unordered list on enter for the new line", async function ({
			page,
		}) {
			const padBody = await getPadBody(page);
	...
	test.describe("Pressing Tab in an UL increases and decreases indentation", function () {
		test("indent and de-indent list item with keypress", async function ({
			page,
		}) {
			const padBody = await getPadBody(page);
			await clearPadContent(page);
	...
	test.describe(
		...
		function () {
			test("indent and de-indent list item with indent button", async function ({
				page,
			}) {
				const padBody = await getPadBody(page);
