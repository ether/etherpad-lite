...
test.describe("height regression after ace.js refactoring", function () {
	test("clientHeight should equal scrollHeight with few lines", async function ({
		page,
	}) {
	...
	test("client height should be less than scrollHeight with many lines", async function ({
		page,
	}) {
		const padBody = await getPadBody(page);
		await padBody.click();
