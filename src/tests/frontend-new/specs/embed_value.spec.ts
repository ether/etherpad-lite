...
test.describe("embed links", function () {
	...
	const checkiFrameCode = async function (
		embedCode: string,
		readonly: boolean,
		page: Page,
	) {
	...
	test.describe("read and write", function () {
		...
		test("is an iframe with the correct url parameters and correct size", async function ({
			page,
		}) {
			const shareButton = page.locator(".buttonicon-embed");
			await shareButton.click();
	...
	test.describe("when read only option is set", function () {
		...
		test("the embed as iframe code is an iframe with the correct url parameters and correct size", async function ({
			page,
		}) {
			// open share dropdown
			const shareButton = page.locator(".buttonicon-embed");
