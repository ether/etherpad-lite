...
AttributeManager.prototype = _(AttributeManager.prototype).extend({
  ...
  */
	setAttributesOnRange(start, end, attribs) {
		...
		for (let row = start[0]; row <= end[0]; row++) {
			...
			const rowChangeset = this._setAttributesOnRangeByLine(
				row,
				startCol,
				endCol,
				attribs,
	...
	_findRowRange(row, start, end) {
		...
		const lineLength =
			this.rep.lines.offsetOfIndex(row + 1) -
			this.rep.lines.offsetOfIndex(row) -
			1;
  ...
  */
	getAttributeOnLine(lineNum, attributeName) {
		...
		if (op == null) return "";
		return (
			AttributeMap.fromString(op.attribs, this.rep.apool).get(attributeName) ||
			""
		);
	},
	...
	getAttributeOnSelection(attributeName, prevChar) {
		...
		const rangeHasAttrib = (selStart, selEnd) => {
			...
			if (selStart[0] !== selEnd[0]) {
				...
				let hasAttrib = rangeHasAttrib(selStart, [
					selStart[0],
  ...
  */
	getAttributesOnPosition(lineNumber, column) {
		...
		for (const currentOperation of Changeset.deserializeOps(aline)) {
			...
			return [
				...attributes.attribsFromString(
					currentOperation.attribs,
					this.rep.apool,
				),
			];
		}
  ...
  */
	getAttributesOnCaret() {
		return this.getAttributesOnPosition(
			this.rep.selStart[0],
			this.rep.selStart[1],
		);
	},
  ...
  */
	...
	removeAttributeOnLine(lineNum, attributeName, attributeValue) {
		...
		const attribs = this.getAttributesOnLine(lineNum).map((attrib) => {
			if (
				attrib[0] === attributeName &&
				(!attributeValue || attrib[0] === attributeValue)
			) {
				found = true;
				return [attrib[0], ""];
		...
		const countAttribsWithMarker = _.chain(attribs)
			.filter((a) => !!a[1])
			.map((a) => a[0])
			.difference(DEFAULT_LINE_ATTRIBUTES)
			.size()
			.value();

		// if we have marker and any of attributes don't need to have marker. we need delete it
		if (hasMarker && !countAttribsWithMarker) {
			ChangesetUtils.buildRemoveRange(
				this.rep,
				builder,
				[lineNum, 0],
				[lineNum, 1],
