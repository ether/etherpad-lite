...
(($) => {
	...
	$.farbtastic = (container, options) => {
		var container = $(container)[0];
		return (
			container.farbtastic ||
	...
	$._farbtastic = function (container, options) {
		...
		fb.mousedown = (event) => {
			...
			fb.circleDrag =
				Math.max(Math.abs(pos.x), Math.abs(pos.y)) > fb.square + 2;
		...
		fb.packDX = (c, a) =>
			"#" + fb.dec2hex(a) + fb.dec2hex(c) + fb.dec2hex(c) + fb.dec2hex(c);
		...
		fb.unpack = (color) => {
			if (color.length == 7) {
				function x(i) {
					return Number.parseInt(color.substring(i, i + 2), 16) / 255;
				}
				return [x(1), x(3), x(5)];
			} else if (color.length == 4) {
				function x(i) {
					return Number.parseInt(color.substring(i, i + 1), 16) / 15;
				}
				return [x(1), x(2), x(3)];
			}
		};
