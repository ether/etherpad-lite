...
((global, factory) => {
	...
	var flat = arr.flat
		? (array) => arr.flat.call(array)
		: (array) => arr.concat.apply([], array);
	...
	jQuery.escapeSelector = (sel) => (sel + "").replace(rcssescape, fcssescape);
	...
	(() => {
		...
		function createInputPseudo(type) {
			return (elem) => nodeName(elem, "input") && elem.type === type;
		}
		...
		function setDocument(node) {
			...
			// Class
			Expr.find.CLASS = (className, context) => {
				if (
	...
	---------------------------------------------------------------------- */
		...
		find.matches = (expr, elements) => find(expr, null, null, elements);
		...
		Expr = jQuery.expr = {
			...
			*/
			...
			filter: {
				TAG: (nodeNameSelector) => {
					...
					return nodeNameSelector === "*"
						? () => true
						: (elem) => nodeName(elem, expectedNodeName);
				},
			...
			pseudos: {
				...
				has: markFunction(
					(selector) => (elem) => find(selector, elem).length > 0,
				),
				...
				root: (elem) => elem === documentElement,
				...
				eq: createPositionalPseudo((_matchIndexes, length, argument) => [
		...
		function matcherFromTokens(tokens) {
			var checkContext,
				...
				matchAnyContext = addCombinator(
					(elem) => indexOf.call(checkContext, elem) > -1,
					implicitRelative,
					true,
				),
				matchers = [
	...
	jQuery.each(
		{
			...
			parentsUntil: (elem, _i, until) => dir(elem, "parentNode", until),
			...
			nextUntil: (elem, _i, until) => dir(elem, "nextSibling", until),
			prevUntil: (elem, _i, until) => dir(elem, "previousSibling", until),
	...
	jQuery.Deferred.exceptionHook = (error, asyncError) => {
		// Support: IE 8 - 9 only
		...
		if (toType(key) === "object") {
			...
			if (bulk) {
				...
				if (raw) {
					...
					bulk = fn;
					fn = (elem, _key, value) => bulk.call(jQuery(elem), value);
				}
			}
	...
	var isAttached = (elem) => jQuery.contains(elem.ownerDocument, elem),
	...
	var isHiddenWithinTree = (elem, el) => {
	...
	jQuery.each({ focus: "focusin", blur: "focusout" }, (type, delegateType) => {
		...
		jQuery.event.special[type] = {
			...
			_default: (event) => dataPriv.get(event.target, type),
	...
	var swap = (elem, options, callback) => {
		var ret,
			name,
			old = {};
		...
		jQuery.extend(support, {
			...
			reliableTrDimensions: () => {
				...
				if (reliableTrDimensionsVal == null) {
					...
					reliableTrDimensionsVal =
						Number.parseInt(trStyle.height, 10) +
							Number.parseInt(trStyle.borderTopWidth, 10) +
							Number.parseInt(trStyle.borderBottomWidth, 10) ===
						tr.offsetHeight;
		...
		if (!xml || parserErrorElem) {
			jQuery.error(
				"Invalid XML: " +
					(parserErrorElem
						? jQuery
								.map(parserErrorElem.childNodes, (el) => el.textContent)
								.join("\n")
						: data),
			);
		}
	...
	jQuery._evalUrl = (url, options, doc) =>
		jQuery.ajax({
			url: url,
	...
	jQuery.trim = (text) =>
		text == null ? "" : (text + "").replace(rtrim, "$1");
