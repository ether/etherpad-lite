...
const getCollabClient = (
	...
	const handleUserChanges = () => {
		...
		if (!getSocket() || channelState === "CONNECTING") {
			if (
				...
				now - initialStartConnectTime > 20000
			) {
	...
	const handleMessageFromServer = (evt) => {
		...
		if (msg.type === "NEW_CHANGES") {
			...
			serverMessageTaskQueue.enqueue(() => {
				...
				if (newRev !== rev + 1) {
					window.console.warn(
						`bad message revision on CLIENT_RECONNECT: ${newRev} not ${
							rev + 1
						}`,
					);
					// setChannelState("DISCONNECTED", "badmessage_acceptcommit");
	...
	editor.setBaseAttributedText(
		serverVars.initialAttributedText,
		serverVars.apool,
	);
