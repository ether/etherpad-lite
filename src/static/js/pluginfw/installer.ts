...
import type { PackageData, PackageInfo } from "../../../node/types/PackageInfo";
...
export const pluginInstallPath = path.join(
	settings.root,
	"src",
	"plugin_packages",
);
export const node_modules = path.join(
	findEtherpadRoot(),
	"src",
	"node_modules",
);

export const installedPluginsPath = path.join(
	settings.root,
...
const migratePluginsFromNodeModules = async () => {
	...
	const [{ dependencies = {} }] = JSON.parse(
		await runCmd(cmd, { stdio: [null, "string"] }),
	);
	...
	await Promise.all(
		Object.entries(dependencies)
			.filter(
				([pkg, info]) =>
					pkg.startsWith(plugins.prefix) && pkg !== "ep_etherpad-lite",
			)
			.map(async ([pkg, info]) => {
				...
				if (!_info.resolved) {
					...
					await linkInstaller.installFromPath(
						`${findEtherpadRoot()}/node_modules/${pkg}`,
					);
				} else {
...
export const checkForMigration = async () => {
		...
		.then(async (err) => {
			...
			for (const file of files) {
				...
				try {
					...
					logger.debug(
						`create symlink for ${file} to ${path.join(
							node_modules,
							moduleName,
						)}`,
					);
					await fs.symlink(
						path.join(pluginInstallPath, file),
						path.join(node_modules, moduleName),
						"dir",
					);
				}
	...
	for (const plugin of installedPlugins.plugins) {
		if (
			plugin.name.startsWith(plugins.prefix) &&
...
export const getAvailablePlugins = (maxCacheAge: number | false) => {
	...
	return new Promise<MapArrayType<PackageInfo>>(async (resolve, reject) => {
		...
		if (
			availablePlugins &&
			maxCacheAge &&
			nowTimestamp - cacheTimestamp <= maxCacheAge
		) {
			return resolve(availablePlugins);
		}

		await axios
			.get("https://static.etherpad.org/plugins.json", { headers })
...
export const search = (searchTerm: string, maxCacheAge: number) =>
	getAvailablePlugins(maxCacheAge).then(
