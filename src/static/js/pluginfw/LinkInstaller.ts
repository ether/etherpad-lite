...
import { type IPluginInfo, PluginManager } from "live-plugin-manager";
...
export class LinkInstaller {
	...
	public async installPlugin(pluginName: string, version?: string) {
		if (version) {
			const installedPlugin = await this.livePluginManager.install(
				pluginName,
				version,
			);
	...
	private async removeSubDependency(_name: string, dependency: string) {
		if (this.dependenciesMap.has(dependency)) {
			console.debug(
				`Dependency ${dependency} is still being used by other plugins`,
			);
			return;
		}
		...
		try {
			const json: IPluginInfo = JSON.parse(
				readFileSync(
					pathToFileURL(
						path.join(pluginInstallPath, dependency, "package.json"),
					),
				) as unknown as string,
			);
			if (json.dependencies) {
				for (const [subDependency] of Object.entries(json.dependencies)) {
	...
	private async addSubDependency(plugin: string, dependency: string) {
		if (this.dependenciesMap.has(dependency)) {
			...
			try {
				...
				console.error(
					`Error reading package.json ${err} for ${pathToFileURL(
						path.join(pluginInstallPath, dependency, "package.json"),
					).toString()}`,
				);
			}
		...
		try {
			...
			try {
				if (dependency.startsWith("@")) {
					...
					symlinkSync(
						path.join(pluginInstallPath, dependency),
						path.join(node_modules, newDependency),
						"dir",
					);
				} else {
					symlinkSync(
