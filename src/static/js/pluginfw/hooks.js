...
const checkDeprecation = (hook) => {
	...
	console.warn(
		`${hook.hook_name} hook used by the ${hook.part.plugin} plugin ` +
...
const callHookFnSync = (hook, context) => {
	...
	const settle = (err, val, how) => {
		...
		if (outcome != null) {
			...
			const msg =
				`DOUBLE SETTLE BUG IN HOOK FUNCTION (plugin: ${hook.part.plugin}, ` +
			...
			if (
				state !== outcome.state ||
				(err == null ? val !== outcome.val : err !== outcome.err)
			) {
		...
		if (val && typeof val.then === "function") {
			console.error(
				`PROHIBITED PROMISE BUG IN HOOK FUNCTION (plugin: ${hook.part.plugin}, ` +
					`function name: ${hook.hook_fn_name}, hook: ${hook.hook_name}): ` +
	...
	if (val === undefined) {
		...
		if (hook.hook_fn.length >= 3) {
			console.error(
				`UNSETTLED FUNCTION BUG IN HOOK FUNCTION (plugin: ${hook.part.plugin}, ` +
					`function name: ${hook.hook_fn_name}, hook: ${hook.hook_name}): ` +
...
exports.callAll = (hookName, context) => {
	...
	return flatten1(
		hooks.map((hook) => normalizeValue(callHookFnSync(hook, context))),
	);
};
...
exports.aCallAll = async (hookName, context, cb = null) => {
	if (cb != null)
		return await attachCallback(exports.aCallAll(hookName, context), cb);
	if (context == null) context = {};
	const hooks = pluginDefs.hooks[hookName] || [];
	const results = await Promise.all(
		hooks.map(async (hook) =>
			normalizeValue(await callHookFnAsync(hook, context)),
		),
	);
...
exports.aCallFirst = async (hookName, context, cb = null, predicate = null) => {
	if (cb != null) {
		return await attachCallback(
			exports.aCallFirst(hookName, context, null, predicate),
			cb,
		);
	}
	if (context == null) context = {};
