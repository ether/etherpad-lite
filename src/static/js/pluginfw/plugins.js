...
const sortHooks = (hookSetName, hooks) => {
	for (const [pluginName, def] of Object.entries(defs.plugins)) {
		for (const part of def.parts) {
			for (const [hookName, hookFnName] of Object.entries(
				part[hookSetName] || {},
			)) {
...
exports.formatHooks = (hookSetName, html) => {
	...
	for (const [hookName, hookEntry] of hooks) {
		...
		for (const [pluginName, pluginEntry] of sortedHookEntry) {
			lines.push(
				html ? `    <dt>${pluginName}:</dt><dd><dl>` : `    ${pluginName}:`,
			);
...
exports.update = async () => {
	...
	await Promise.all(
		Object.keys(packages).map(async (pluginName) => {
	...
	defs.hooks = pluginUtils.extractHooks(
		defs.parts,
		"hooks",
		exports.pathNormalization,
	);
	defs.loaded = true;
	await Promise.all(
		Object.keys(defs.plugins).map(async (p) => {
...
const loadPlugin = async (packages, pluginName, plugins, parts) => {
	...
	try {
		...
		try {
			...
			logger.error(
				`Unable to parse plugin definition file ${pluginPath}: ${
					err.stack || err
				}`,
			);
		}
		...
		logger.error(
			`Unable to load plugin definition file ${pluginPath}: ${
				err.stack || err
			}`,
		);
	}
...
const sortParts = (parts) =>
	tsort(partsToParentChildList(parts))
