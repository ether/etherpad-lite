...
const disabledHookReasons = {
	hooks: {
		indexCustomInlineScripts:
			"The hook makes it impossible to use a Content Security Policy " +
...
const extractHooks = (parts, hookSetName, normalizer) => {
	...
	for (const part of parts) {
		for (const [hookName, regHookFnName] of Object.entries(
			part[hookSetName] || {},
		)) {
			 ...
			 * npm... */
			const hookFnName = normalizer
				? normalizer(part, regHookFnName, hookName)
				: regHookFnName;

			const disabledReason = (disabledHookReasons[hookSetName] || {})[hookName];
			if (disabledReason) {
				console.error(
					`Hook ${hookSetName}/${hookName} is disabled. Reason: ${disabledReason}`,
				);
				console.error(
					`The hook function ${hookFnName} from plugin ${part.plugin} ` +
						"will never be called, which may cause the plugin to fail",
				);
				console.error(
					`Please update the ${part.plugin} plugin to not use the ${hookName} hook`,
				);
				return;
			}
			let hookFn;
			try {
				...
				console.error(
					`Failed to load hook function "${hookFnName}" for plugin "${part.plugin}" ` +
...
exports.clientPluginNames = () => {
	const clientPluginNames = defs.parts
		.filter((part) =>
			Object.prototype.hasOwnProperty.call(part, "client_hooks"),
		)
