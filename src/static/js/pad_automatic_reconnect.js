exports.showCountDownTimerToReconnectOnModal = ($modal, pad) => {
	if (
		clientVars.automaticReconnectionTimeout &&
		$modal.is(".with_reconnect_timer")
	) {
...
const createCountDownElementsIfNecessary = ($modal) => {
	...
	if (elementsDoNotExist) {
		...
		const $reconnectTimerMessage = $("<p>")
			...
			.append(" ")
			.append($("<span>").addClass("timetoexpire"));
...
const toggleAutomaticReconnectionOption = (
	$modal,
	disableAutomaticReconnect,
) => {
	$modal
		.find("#cancelreconnect, .reconnecttimer")
		.toggleClass("hidden", disableAutomaticReconnect);
...
CountDownTimer.prototype.start = function () {
	...
	let diff;
	const timer = () => {
		diff = this.duration - Math.floor((Date.now() - start) / 1000);

		if (diff > 0) {
			this.timeoutId = setTimeout(timer, this.granularity);
			this.tick(diff);
		} else {
			this.running = false;
			this.tick(0);
