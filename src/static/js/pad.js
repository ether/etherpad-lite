...
const padconnectionstatus =
	require("./pad_connectionstatus").padconnectionstatus;
...
const sendClientReady = (isReconnect) => {
	let padId = document.location.pathname.substring(
...
const pad = {
	...
	_afterHandshake() {
		...
		const postAceInit = () => {
			...
			$("#viewfontmenu")
				.val(padcookie.getPref("padFontFamily"))
				.niceSelect("update");
		...
		if (
			settings.globalUserColor !== false &&
			colorutils.isCssHex(settings.globalUserColor)
		) {
	...
	handleClientMessage: (msg) => {
		if (msg.type === "suggestUserName") {
			if (
				msg.unnamedId === pad.myUserInfo.userId &&
				msg.newName &&
				!pad.myUserInfo.name
			) {
	...
	forceReconnect: () => {
		...
		pad.diagnosticInfo.collabDiagnosticInfo =
			pad.collabClient.getDiagnosticInfo();
		$("form#reconnectform input.diagnosticInfo").val(
			JSON.stringify(pad.diagnosticInfo),
		);
