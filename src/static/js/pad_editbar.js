...
exports.padeditbar = new (class {
	...
	init() {
		...
		$(window).on(
			"resize",
			_.debounce(() => this.checkAllIconsAreDisplayedInToolbar(), 100),
		);
		...
		try {
			...
			if (moduleName === "none") {
				for (const thisModuleName of this.dropdowns) {
					...
					const isAForceReconnectMessage =
						module.find("button#forcereconnect:visible").length > 0;
	...
	checkAllIconsAreDisplayedInToolbar() {
		...
		if (
			(menuLeft &&
				menuLeft.scrollWidth > $(".toolbar").width() - menuRightWidth) ||
			$(".toolbar").width() < 1000
		) {
