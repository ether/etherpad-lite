	...
	insert(entry) {
		...
		while (newNode.levels === 0 || Math.random() < 0.01) {
			...
			if (lvl === pNodes.length) {
				...
				this._skipList._start.downSkips[lvl] =
					this._skipList._keyToNodeMap.size + 1;
	...
	delete() {
		...
		for (let i = 0; i < this.nodes.length; i++) {
			if (i < elem.levels) {
				...
				const totalWidthSkip =
					up.downSkipWidths[i] + elem.downSkipWidths[i] - elemWidth;
...
class SkipList {
	...
	_getNodeAtOffset(targetOffset) {
		...
		if (n === this._end) {
			return targetOffset === this._totalWidth
				? this._end.upPtrs[0] || null
				: null;
		}
		return n;
	}
	...
	splice(start, deleteCount, newEntryArray) {
		...
		if (start + deleteCount > this._keyToNodeMap.size) {
			console.warn(
				`splice(${start}, ${deleteCount}, ...), N=${this._keyToNodeMap.size}`,
			);
			console.warn(
				"%s %s %s",
				typeof start,
				typeof deleteCount,
				typeof this._keyToNodeMap.size,
			);
			console.trace();
		}
	...
	setEntryWidth(entry, width) {
		entry.width = width;
		this._totalWidth += this._keyToNodeMap
			.get(entry.key)
			.propagateWidthChange();
	}
	totalWidth() {
