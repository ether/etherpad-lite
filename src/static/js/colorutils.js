...
// "#ffffff" or "#fff"
colorutils.isCssHex = (cssColor) =>
	/^#([0-9a-f]{3}|[0-9a-f]{6})$/i.test(cssColor);
...
colorutils.triple2css = (triple) => {
	...
	return `#${floatToHex(triple[0])}${floatToHex(triple[1])}${floatToHex(
		triple[2],
	)}`;
};
...
colorutils.scale = (v, bot, top) =>
	colorutils.clamp(bot + v * (top - bot), 0, 1);
colorutils.unscale = (v, bot, top) =>
	colorutils.clamp((v - bot) / (top - bot), 0, 1);
...
colorutils.luminosity = (c) => c[0] * 0.3 + c[1] * 0.59 + c[2] * 0.11;
...
colorutils.textColorFromBackgroundColor = (bgcolor, skinName) => {
	...
	return colorutils.luminosity(colorutils.css2triple(bgcolor)) < 0.5
		? white
		: black;
};
