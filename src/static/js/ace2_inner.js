...
function Ace2Inner(editorInfo, cssManagers) {
	const makeChangesetTracker =
		require("./changesettracker").makeChangesetTracker;
	const colorutils = require("./colorutils").colorutils;
	const makeContentCollector =
		require("./contentcollector").makeContentCollector;
	...
	const setAuthorStyle = (author, info) => {
		const authorSelector = getAuthorColorClassSelector(
		...
		if (!info) {
			...
			const textColor = colorutils.textColorFromBackgroundColor(
				bgcolor,
				parent.parent.clientVars.skinName,
			);
			const styles = [
	...
	const getAuthorClassName = (author) =>
	...
	const getAuthorColorClassSelector = (oneClassName) =>
		`.authorColors .${oneClassName}`;
	...
	const isScrollableEditEvent = (eventType) =>
		!_nonScrollableEditEvents[eventType];
		...
		try {
			...
			if (cleanExit) {
				...
				if (cs.domClean && cs.type !== "setup") {
					...
					if (
						(cs.docTextChanged || cs.userChangedSelection) &&
						isScrollableEditEvent(cs.type)
					) {
	...
	const importAText = (atext, apoolJsonObj, undoable) => {
		...
		if (apoolJsonObj) {
			...
			atext.attribs = Changeset.moveOpsToNewPool(
				atext.attribs,
				wireApool,
				rep.apool,
			);
	...
	editorInfo.ace_setProperty = (key, value) => {
		...
		const setters = {
			wraps: setWraps,
			showsauthorcolors: (val) =>
				document.body.classList.toggle("authorColors", !!val),
			showsuserselections: (val) =>
				document.body.classList.toggle("userSelections", !!val),
			showslinenumbers: (value) => {
				hasLineNumbers = !!value;
				sideDiv.parentNode.classList.toggle(
					"line-numbers-hidden",
					!hasLineNumbers,
				);
				fixView();
	...
	editorInfo.ace_prepareUserChangeset = () =>
		changesetTracker.prepareUserChangeset();
	...
	const recolorLinesInRange = (startChar, endChar) => {
		...
		while (lineEntry && lineStart < endChar) {
			...
			if (
				(rep.selStart && rep.selStart[0] === lineIndex) ||
	...
	const getSpansForLine = (
		...
		if (text.length === 0) {
			...
			let filteredFunc = linestylefilter.getFilterStack(
				text,
				textAndClassFunc,
				browser,
			);
	...
	const observeChangesAroundNode = (node) => {
		...
		if (!isNodeDirty(node)) {
			...
			hasAdjacentDirtyness =
				(prevSib && isNodeDirty(prevSib)) || (nextSib && isNodeDirty(nextSib));
		} else {
	...
	const incorporateUserChanges = () => {
		...
		while (j < dirtyRanges.length) {
			...
			if (
				!(
					(a === 0 || getCleanNodeByKey(rep.lines.atIndex(a - 1).key)) &&
					(b === rep.lines.length() ||
		...
		if (!dirtyRangesCheckOut) {
			for (const bodyNode of document.body.childNodes) {
				if (
					bodyNode.tagName &&
					(!bodyNode.id || !rep.lines.containsKey(bodyNode.id))
				) {
		...
		while (i < dirtyRanges.length) {
			...
			b = range[1];
			let firstDirtyNode =
				(a === 0 && document.body.firstChild) ||
				getCleanNodeByKey(rep.lines.atIndex(a - 1).key).nextSibling;
			firstDirtyNode =
				firstDirtyNode && isNodeDirty(firstDirtyNode) && firstDirtyNode;

			let lastDirtyNode =
				(b === rep.lines.length() && document.body.lastChild) ||
				getCleanNodeByKey(rep.lines.atIndex(b).key).previousSibling;

			lastDirtyNode =
				lastDirtyNode && isNodeDirty(lastDirtyNode) && lastDirtyNode;
			if (firstDirtyNode && lastDirtyNode) {
				const cc = makeContentCollector(
					isStyled,
					browser,
					rep.apool,
					className2Author,
		...
		if (selection && !selStart) {
			...
			selStart =
				selStartFromHook == null || selStartFromHook.length === 0
					? getLineAndCharForPoint(selection.startPoint)
					: selStartFromHook;
		}
		if (selection && !selEnd) {
			...
			selEnd =
				selEndFromHook == null || selEndFromHook.length === 0
					? getLineAndCharForPoint(selection.endPoint)
					: selEndFromHook;
		}
	...
	const isDefaultLineAttribute = (aname) =>
		AttributeManager.DEFAULT_LINE_ATTRIBUTES.indexOf(aname) !== -1;
	...
	const caretDocChar = () =>
		rep.lines.offsetOfIndex(caretLine()) + caretColumn();
	...
	const handleReturnIndentation = () => {
		...
		if (isCaret() && caretColumn() === 0 && caretLine() > 0) {
			...
			const cs = Changeset.builder(rep.lines.totalWidth())
				.keep(rep.lines.offsetOfIndex(lineNum), lineNum)
				.insert(theIndent, [["author", thisAuthor]], rep.apool)
				.toString();
			performDocumentApplyChangeset(cs);
			performSelectionChange(
				[lineNum, theIndent.length],
				[lineNum, theIndent.length],
			);
		}
	};
	...
	const performDocumentApplyChangeset = (changes, insertsAfterSelection) => {
		const domAndRepSplice = (startLine, deleteCount, newLineStrings) => {
			...
			insertDomLines(
				nodeToAddAfter,
				lineEntries.map((entry) => entry.domInfo),
			);
		...
		if (rep.selStart && rep.selEnd) {
			...
			const result = Changeset.characterRangeFollow(
				changes,
				selStartChar,
				selEndChar,
	...
	const doRepApplyChangeset = (changes, insertsAfterSelection) => {
		...
		if (editEvent.eventType === "nonundoable") {
			if (!editEvent.changeset) {
				...
				editEvent.changeset = Changeset.compose(
					editEvent.changeset,
					changes,
					rep.apool,
				);
			...
			const inverseChangeset = Changeset.inverse(
				changes,
				{
					get: (i) => `${rep.lines.atIndex(i).text}\n`,
					length: () => rep.lines.length(),
			...
			if (!editEvent.backset) {
				...
				editEvent.backset = Changeset.compose(
					inverseChangeset,
					editEvent.backset,
					rep.apool,
				);
	...
	const getAttributeOnSelection = (attributeName, prevChar) => {
		...
		const isNotSelection =
			rep.selStart[0] === rep.selEnd[0] && rep.selEnd[1] === rep.selStart[1];
		if (isNotSelection) {
			if (prevChar) {
		...
		const withIt = new AttributeMap(rep.apool)
		...
		const rangeHasAttrib = (selStart, selEnd) => {
			...
			if (selStart[0] !== selEnd[0]) {
				...
				hasAttrib =
					hasAttrib &&
					rangeHasAttrib(selStart, [
						selStart[0],
						rep.lines.atIndex(selStart[0]).text.length,
					]);
				...
				for (let n = selStart[0] + 1; n < selEnd[0]; n++) {
					hasAttrib =
						hasAttrib &&
						rangeHasAttrib([n, 0], [n, rep.lines.atIndex(n).text.length]);
				}

				// for the last, potentially partial, line
				hasAttrib =
					hasAttrib && rangeHasAttrib([selEnd[0], 0], [selEnd[0], selEnd[1]]);
		...
		for (let n = selStartLine; n <= selEndLine; n++) {
			...
			for (const op of Changeset.deserializeOps(rep.alines[n])) {
				...
				if (!hasIt(op.attribs)) {
					...
					if (
						!(
							opEndInLine <= selectionStartInLine ||
	...
	const doRepLineSplice = (startLine, deleteCount, newLineEntries) => {
		...
		rep.alltext =
			rep.alltext.substring(0, startOldChar) +
			newText +
			rep.alltext.substring(endOldChar, rep.alltext.length);
	};

	const doIncorpLineSplice = (
		startLine,
		deleteCount,
		newLineEntries,
		lineAttribs,
		...
		if (hints && hints.selStart) {
			selStartHintChar =
				rep.lines.offsetOfIndex(hints.selStart[0]) +
				hints.selStart[1] -
				oldRegionStart;
		}
		if (hints && hints.selEnd) {
			selEndHintChar =
				rep.lines.offsetOfIndex(hints.selEnd[0]) +
				hints.selEnd[1] -
				oldRegionStart;
		}
		...
		const oldAttribs = rep.alines
			.slice(startLine, startLine + deleteCount)
			.join("");
		...
		let shortOldText = oldText.substring(
			commonStart,
			oldText.length - commonEnd,
		);
		let shortNewText = newText.substring(
			commonStart,
		...
		// be very defensive
		if (
			shortOldText.charAt(shortOldText.length - 1) === "\n" &&
		...
		if (!(shortOldText.length === 0 && shortNewText.length === 0)) {
			...
			if (justApplyStyles) {
				...
				const incorpedAttribClearer = cachedStrFunc((oldAtts) =>
					Changeset.mapAttribNumbers(oldAtts, (n) => {
				...
				if (spliceEndLineStart > spliceStart) {
					builder.remove(
						spliceEndLineStart - spliceStart,
	...
	const analyzeChange = (
		...
		// Etherpad
		const incorpedAttribFilter = (anum) =>
			!isDefaultLineAttribute(rep.apool.getAttribKey(anum));
		...
		const oldARuns = attribRuns(
			Changeset.filterAttribNumbers(oldAttribs, incorpedAttribFilter),
		);
		const newARuns = attribRuns(
			Changeset.filterAttribNumbers(newAttribs, incorpedAttribFilter),
		);
		...
		while (commonStart < minLen) {
			if (
				oldText.charAt(commonStart) === newText.charAt(commonStart) &&
		...
		while (commonEnd < minLen) {
			if (commonEnd === 0) {
				...
				oldText.charAt(oldLen - 1 - commonEnd) ===
	...
	const repSelectionChange = (selectStart, selectEnd, focusAtStart) => {
		...
		if (
			!equalLineAndChars(rep.selStart, selectStart) ||
			...
			if (!docTextChanged) {
				const isScrollableEvent =
					!isPadLoading(currentCallStack.type) &&
	...
	const isPadLoading = (t) =>
		t === "setup" || t === "setBaseText" || t === "importText";

	const updateStyleButtonState = (attribName, hasStyleOnRepSelection) => {
		const $formattingButton = parent.parent
			.$(`[data-key="${attribName}"]`)
			.find("a");
		$formattingButton.toggleClass(SELECT_BUTTON_CLASS, hasStyleOnRepSelection);
	};

	const attribIsFormattingStyle = (attribName) =>
		FORMATTING_STYLES.indexOf(attribName) !== -1;
	...
	const doCreateDomLine = (nonEmpty) =>
		domline.createDomLine(nonEmpty, doesWrap, browser, document);
	...
	for (const element of hooks.callAll("aceRegisterBlockElements"))
	...
	const setLineListType = (lineNum, listType) => {
		if (listType === "") {
			documentAttributeManager.removeAttributeOnLine(
				lineNum,
				listAttributeName,
			);
	...
	const doIndentOutdent = (isOut) => {
		...
		for (let n = firstLine; n <= lastLine; n++) {
			...
			const newLevel = Math.max(
				0,
				Math.min(MAX_LIST_LEVEL, level + (isOut ? -1 : 1)),
			);
			if (level !== newLevel) {
			...
			if (!handled) {
				if (isCaret()) {
					...
					if (caretColumn() <= lineEntry.lineMarker) {
						...
						const thisLineHasMarker =
							documentAttributeManager.lineHasMarker(theLine);
						...
						if (docChar > 0) {
							if (evt.metaKey || evt.ctrlKey || evt.altKey) {
								...
								while (
									deleteBackTo > lineEntry.lineMarker &&
	...
	const handleKeyEvent = (evt) => {
		...
		const isTypeForSpecialKey =
			browser.safari || browser.chrome || browser.firefox
				? type === "keydown"
				: type === "keypress";
		const isTypeForCmdKey =
			browser.safari || browser.chrome || browser.firefox
				? type === "keydown"
				: type === "keypress";
		...
		inCallStackIfNecessary("handleKeyEvent", function () {
			...
			if (!stopped) {
				...
				if (
					...
					const firstEditbarElement = parent.parent
						.$("#editbar")
						.children("ul")
						.first()
						.children()
				...
				if (
					...
					if (alineAttrs) {
						for (const op of Changeset.deserializeOps(alineAttrs)) {
							const authorId = AttributeMap.fromString(op.attribs, apool).get(
								"author",
							);
					...
					const idToName = new Map(
						parent.parent.pad.userList().map((a) => [a.userId, a.name]),
					);
					const myId = parent.parent.clientVars.userId;
					const authors = [...authorIds].map((id) =>
						id === myId ? "me" : idToName.get(id) || "unknown",
					);
					...
					parent.parent.$.gritter.add({
						...
						text:
							authors.length === 0
								...
								: authors.length === 1
									? `The author of this line is ${authors[0]}`
				...
				if (
					...
					evt.preventDefault();
					const originalBackground = parent.parent
						.$("#revisionlink")
						.css("background");
				...
				if (
					...
					scheduler.setTimeout(() => {
						...
						const numberOfLinesInViewport =
							newVisibleLineRange[1] - newVisibleLineRange[0];
			...
			if (
				thisKeyDoesntTriggerNormalize ||
				isFirefoxHalfCharacter ||
	...
	editorInfo.ace_performDocumentReplaceCharRange =
		performDocumentReplaceCharRange;
	...
	const teardown = () => {
		for (const a of _teardownActions) a();
	};

	let inInternationalComposition = null;
	editorInfo.ace_getInInternationalComposition = () =>
		inInternationalComposition;
	...
	const bindTheEventHandlers = () => {
		...
		$(document.body).on("auxclick", (e) => {
			if (
				...
				(e.target.a || e.target.localName === "a")
			) {
	...
	const getInnerHeight = () => {
		const h = browser.opera
			? outerWin.innerHeight
			: outerDoc.documentElement.clientHeight;
		if (h) return h;

		// deal with case where iframe is hidden, hope that
		// style.height of iframe container is set in px
		return Number(
			editorInfo.frame.parentNode.style.height.replace(/[^0-9]/g, "") || 0,
		);
	};
	...
	const scrollSelectionIntoView = () => {
		...
		if (!doesWrap) {
			...
			if (browserSelection) {
				const focusPoint = browserSelection.focusAtStart
					? browserSelection.startPoint
	...
	const getLineListType = (lineNum) =>
		documentAttributeManager.getAttributeOnLine(lineNum, listAttributeName);
	...
	const doInsertList = (type) => {
		...
		for (let n = firstLine; n <= lastLine; n++) {
			...
			if (togglingOn) {
				mods.push([
					n,
					allLinesAreList ? `indent${level}` : t ? type + level : `${type}1`,
				]);
			} else {
				// scrap the entire indentation and list type
				if (level === 1) {
					// if outdending but are the first item in the list then outdent
	...
	const updateLineNumbers = () => {
		...
		const defaultLineHeight = Number.parseInt(
			innerdocbodyStyles["line-height"],
		);
		...
		for (const [i, sideDivLine] of Array.prototype.entries.call(
			sideDivInner.children,
		)) {
	...
	documentAttributeManager = new AttributeManager(
		rep,
		performDocumentApplyChangeset,
	);

	editorInfo.ace_performDocumentApplyAttributesToRange = (...args) =>
		documentAttributeManager.setAttributesOnRange(...args);
