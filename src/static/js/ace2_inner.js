...
function Ace2Inner() {
    ...
    function setAuthorStyle(author, info)
    {
        ...
        if (_.any(authorStyleSet, function (it) {
            return it
        }))
        {
            return
        ...
        else
        {
            if (info.bgcolor)
            {
                ...
                // author color
                authorStyle.borderBottom = '2px solid ' + bgcolor;
                parentAuthorStyle.BorderBottom = '2px solid ' + bgcolor;
    ...
    editorInfo.ace_getAuthorClassName = getAuthorClassName;
    ...
    function recolorLinesInRange(startChar, endChar, isTimeUp, optModFunc)
    {
        if (endChar <= startChar)
            return;
        if (startChar < 0 || startChar >= rep.lines.totalWidth())
            return;
    ...
    function performDocumentApplyChangeset(changes, insertsAfterSelection)
    {
        ...
        var linesMutatee = {
            splice: function (start, numRemoved, newLinesVA)
            {
                ...
                domAndRepSplice(start, numRemoved, _.map(args, function (s) {
                    return s.slice(0, -1);
                }), null);
            },
            get: function (i)
    ...
    function doRepApplyChangeset(changes, insertsAfterSelection)
    {
        ...
        if (Changeset.oldLen(changes) != rep.alltext.length)
            throw new Error("doRepApplyChangeset length mismatch: " + Changeset.oldLen(changes) + "/" + rep.alltext.length);
    ...
    function getDirtyRanges()
    {
        ...
        function isConsecutive(i)
        {
            if (isConsecutiveCache[i] === undefined)
            {
                ...
                isConsecutiveCache[i] = (function ()
                {
                    ...
                    if ((!a) || (!b))
                        return false; // violates precondition
                    if ((a === true) && (b === true))
        ...
        function rangeForLine(i)
        {
            ...
            _.each(cleanRanges, function (r, idx)
            {
                ...
                if (i < r[0])
                    return true; // not found, stop looking
                answer = idx;
    ...
    function moveByWordInLine(lineText, initialIndex, forwardNotBack)
    {
        ...
        {
            if (forwardNotBack)
                return lineText.charAt(i);
            else
                return lineText.charAt(i - 1);
        }
    ...
    function getSelection()
    {
        ...
        if (browser.msie)
        {
            ...
            if (selectionParent.ownerDocument != doc)
                return null;
        ...
        if (browser.msie)
        {
            ...
            if (selection)
            {
                ...
                function isEqualToDocumentSelection(rng)
                {
                    ...
                    if (rng2.parentElement().ownerDocument != doc)
                        return false;
                    if (rng.compareEndPoints("StartToStart", rng2) !== 0)
                        return false;
                    if (rng.compareEndPoints("EndToEnd", rng2) !== 0)
                        return false;
                    return true;
                }
    ...
    function fixView()
    {
        ...
        if (newSideDivWidth < MIN_LINEDIV_WIDTH)
            newSideDivWidth = MIN_LINEDIV_WIDTH;
        iframePadLeft = EDIT_BODY_PADDING_LEFT;
        if (hasLineNumbers)
            iframePadLeft += newSideDivWidth + LINE_NUMBER_PADDING_RIGHT;
        ...
        for (var i = 0; i < 2; i++)
        {
            ...
            if (newHeight < viewHeight)
            {
                ...
                if (browser.msie)
                    setIfNecessary(outerWin.document.documentElement.style, 'overflowY', 'auto');
            }
            else
            {
                if (browser.msie)
                    setIfNecessary(outerWin.document.documentElement.style, 'overflowY', 'scroll');
            }
            if (doesWrap)
            {
                newWidth = viewWidth;
    ...
    function getPageHeight()
    {
        ...
        else if (odoc.body.scrollHeight > odoc.body.offsetHeight)
            return odoc.body.scrollHeight;
        else
            return odoc.body.offsetHeight;
    }
    ...
    function getPageWidth()
    {
        ...
        if (win.innerWidth && win.scrollMaxX)
            return win.innerWidth + win.scrollMaxX;
        else if (odoc.body.scrollWidth > odoc.body.offsetWidth)
            return odoc.body.scrollWidth;
        else
            return odoc.body.offsetWidth;
    }
