...
const loadBroadcastSliderJS = (fireWhenAllScriptsAreLoaded) => {
	...
	(() => {
		...
		const updateSliderElements = () => {
			for (let i = 0; i < savedRevisions.length; i++) {
				const position = Number.parseInt(savedRevisions[i].attr("pos"));
				savedRevisions[i].css(
					"left",
					(position * ($("#ui-slider-bar").width() - 2)) /
						(sliderLength * 1.0) -
						1,
				);
			}
		...
		const addSavedRevision = (position, info) => {
			...
			newSavedRevision.css(
				"left",
				(position * ($("#ui-slider-bar").width() - 2)) / (sliderLength * 1.0) -
					1,
			);
		...
		const setSliderPosition = (newpos) => {
			...
			$("#ui-slider-handle").css(
				"left",
				(newpos * ($("#ui-slider-bar").width() - 2)) / (sliderLength * 1.0),
			);
			$("a.tlink").map(function () {
				$(this).attr(
					"href",
					$(this).attr("thref").replace("%revision%", newpos),
				);
			});

			$("#revision_label").html(
				html10n.get("timeslider.version", { version: newpos }),
			);

			$("#leftstar, #leftstep").toggleClass("disabled", newpos === 0);
			$("#rightstar, #rightstep").toggleClass(
				"disabled",
				newpos === sliderLength,
			);
		...
		const setAuthors = (authors) => {
			...
			_.each(authors, (author) => {
				if (author) {
					const authorColor =
						clientVars.colorPalette[author.colorId] || author.colorId;
					if (author.name) {
			...
			if (numAnonymous > 0) {
				const anonymousAuthorString = html10n.get("timeslider.unnamedauthors", {
					num: numAnonymous,
				});
		...
		fireWhenAllScriptsAreLoaded.push(() => {
			...
			$("#ui-slider-bar").on("mousedown", (evt) => {
				$("#ui-slider-handle").css(
					"left",
					evt.clientX - $("#ui-slider-bar").offset().left,
				);
			...
			$("#ui-slider-handle").on("mousedown", function (evt) {
				...
				this.currentLoc = Number.parseInt($(this).css("left"));
				sliderActive = true;
				$(document).on("mousemove", (evt2) => {
					...
					$("#revision_label").html(
						html10n.get("timeslider.version", { version }),
					);
				...
				$(document).on("mouseup", (evt2) => {
					...
					if (Number.parseInt($(this).css("left")) < 2) {
						$(this).css("left", "2px");
					} else {
						this.currentLoc = Number.parseInt($(this).css("left"));
					}
			...
			$(".stepper").on("click", function (evt) {
				switch ($(this).attr("id")) {
					...
					case "leftstar": {
						...
						for (let i = 0; i < savedRevisions.length; i++) {
							const pos = Number.parseInt(savedRevisions[i].attr("pos"));
	...
	BroadcastSlider.onSlider((loc) => {
		$("#viewlatest").html(
			`${
				loc === BroadcastSlider.getSliderLength() ? "Viewing" : "View"
			} latest content`,
		);
	});
