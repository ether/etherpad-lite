...
const eventFired = async (
	...
	await new Promise((resolve, reject) => {
		...
		const errorCb = () => {
			const err = new Error(
				`Ace2Editor.init() error event while waiting for ${event} event`,
			);
...
const Ace2Editor = function () {
	...
	this.exportText = () =>
		loaded ? info.ace_exportText() : "(awaiting init)\n";
	...
	this.prepareUserChangeset = () =>
		loaded ? info.ace_prepareUserChangeset() : null;
	...
	this.init = async function (containerId, initialCode) {
		...
		const skinVariants = clientVars.skinVariants
			.split(" ")
			.filter((x) => x !== "");
		...
		// <html> tag
		outerDocument.documentElement.classList.add(
			"outer-editor",
			"outerdoc",
			...skinVariants,
		);
		...
		outerDocument.body.classList.add(
			"outerdocbody",
			...pluginUtils.clientPluginNames(),
		);
		...
		// <html> tag
		innerDocument.documentElement.classList.add(
			"inner-editor",
			...skinVariants,
		);
		...
		requireKernel.src = absUrl(
			`../static/js/require-kernel.js?v=${clientVars.randomVersionString}`,
		);
		...
		for (const module of ["ace2_inner", "ace2_common"]) {
			...
			script.src = absUrl(
				`../javascripts/lib/ep_etherpad-lite/static/js/${module}.js` +
		...
		innerDocument.head.appendChild(
			innerDocument
				.createRange()
				.createContextualFragment(headLines.join("\n")),
		);
		...
		innerWindow.$ = innerWindow.jQuery =
			require("ep_etherpad-lite/static/js/rjquery").jQuery;

		debugLog("Ace2Editor.init() waiting for plugins");
		await new Promise((resolve, reject) =>
			innerWindow.plugins.ensure((err) =>
