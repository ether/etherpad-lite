...
const paduserlist = (() => {
	const rowManager = (() => {
		...
		const animateStep = () => {
			...
			for (let i = rowsFadingIn.length - 1; i >= 0; i--) {
				...
				if (step <= -OPACITY_STEPS) {
					...
					node
						.empty()
						.append(createUserRowTds(animHeight, row.data))
						.find("td")
					...
					node
						...
						.css(
							"opacity",
							(baseOpacity * (OPACITY_STEPS - -step)) / OPACITY_STEPS,
						)
						.height(animHeight);
				} else if (step === 0) {
		...
		const { scheduleAnimation } = padutils.makeAnimationScheduler(
			animateStep,
			ANIMATION_STEP_TIME,
		...
		const isNameEditable = (data) =>
			!data.name && data.status !== "Disconnected";

		const replaceUserRowContents = (tr, height, data) => {
			const tds = createUserRowTds(height, data);
			if (
				isNameEditable(data) &&
				tr.find("td.usertdname input:enabled").length > 0
			) {
		...
		const createRow = (id, contents, authorId) =>
			$("<tr>").attr("data-authorId", authorId).attr("id", id).append(contents);
		...
		const insertRow = (position, data, animationPower) => {
			...
			if (animationPower === 0) {
				tr = createRow(
					domId,
					createUserRowTds(getAnimationHeight(0), data),
					authorId,
				);
				row.animationStep = 0;
			} else {
				...
				tr = createRow(
					domId,
					createEmptyRowTds(getAnimationHeight(ANIMATION_START)),
					authorId,
				);
			}
	...
	const self = {
		...
		userJoinOrUpdate: (info) => {
			...
			const userData = {};
			userData.color =
				typeof info.colorId === "number"
					? clientVars.colorPalette[info.colorId]
					: info.colorId;
		...
		renderMyUserInfo: () => {
			if (myUserInfo.name) {
				...
				$("#myusernameedit").attr(
					"placeholder",
					html10n.get("pad.userlist.entername"),
				);
			}
			if (colorPickerOpen) {
				$("#myswatchbox")
...
const getColorPickerSwatchIndex = (jnode) =>
	$("#colorpickerswatches li").index(jnode);
...
const closeColorPicker = (accept) => {
	if (accept) {
		...
		if (parts) {
			delete parts[0];
			for (let i = 1; i <= 3; ++i) {
				parts[i] = Number.parseInt(parts[i]).toString(16);
		...
		if (!colorPickerSetup) {
			...
			for (let i = 0; i < palette.length; i++) {
				...
				li.on("click", (event) => {
					...
					const newColorId = getColorPickerSwatchIndex(
						$("#colorpickerswatches .picked"),
					);
