	...
	return (self = {
		...
		setBaseAttributedText: (atext, apoolJsonObj) => {
			aceCallbacksProvider.withCallbacks("setBaseText", (callbacks) => {
				...
				if (apoolJsonObj) {
					...
					baseAText.attribs = Changeset.moveOpsToNewPool(
						baseAText.attribs,
						wireApool,
						apool,
					);
		...
		applyChangesToBase: (c, optAuthor, apoolJsonObj) => {
			...
			aceCallbacksProvider.withCallbacks("applyChangesToBase", (callbacks) => {
				...
				if (submittedChangeset) {
					...
					submittedChangeset = Changeset.follow(
						c,
						oldSubmittedChangeset,
						false,
						apool,
					);
				...
				applyingNonUserChanges = true;
				try {
					callbacks.applyChangesetToDocument(
						postChange,
		...
		applyPreparedChangesetToBase: () => {
			if (!submittedChangeset) {
				...
				throw new Error(
					"applySubmittedChangesToBase: no submitted changes to apply",
				);
			}
		...
		hasUncommittedChanges: () =>
			!!(submittedChangeset || !Changeset.isIdentity(userChangeset)),
	});
};
