...
linestylefilter.getLineStyleFilter = (
	lineLength,
	aline,
	textAndClassFunc,
	apool,
	...
	for (const attribClasses of hooks.callAll(
		"aceAttribClasses",
		linestylefilter.ATTRIB_CLASSES,
	)) {
	...
	const authorColorFunc = (() => {
		...
		const attribsToClasses = (attribs) => {
			...
			for (const [key, value] of attributes.attribsFromString(attribs, apool)) {
				...
				if (
					!isLineAttribMarker &&
					AttributeManager.lineAttributes.indexOf(key) >= 0
				) {
				...
				if (key === "author") {
					...
					const results = hooks.callAll("aceAttribsToClasses", {
						linestylefilter,
						key,
						value,
					});
		...
		return (txt, cls) => {
			const disableAuthColorForThisLine = hooks.callAll(
			...
			const disableAuthors =
				disableAuthColorForThisLine == null ||
				disableAuthColorForThisLine.length === 0
					? false
					: disableAuthColorForThisLine[0];
			while (txt.length > 0) {
...
linestylefilter.getAtSignSplitterFilter = (lineText, textAndClassFunc) => {
	...
	return linestylefilter.textAndClassFuncSplitter(
		textAndClassFunc,
		splitPoints,
	);
};

linestylefilter.getRegexpFilter =
	(regExp, tag) => (lineText, textAndClassFunc) => {
		regExp.lastIndex = 0;
