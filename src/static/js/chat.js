...
exports.chat = (() => {
	...
	return {
		...
		chatAndUsers(fromInitialCall) {
			...
			$("#users, .sticky-container").toggleClass(
				"chatAndUsers popup-show stickyUsers",
				userAndChat,
			);
		...
		async addMessage(msg, increment, isHistoryAdd) {
			...
			const authorClass = (authorId) =>
				`author-${authorId.replace(/[^a-y0-9]/g, (c) => {
					if (c === ".") return "-";
			...
			const ctx = {
				authorName:
					msg.displayName != null
						? msg.displayName
						: html10n.get("pad.userlist.unnamed"),
				author: msg.authorId,
			...
			if (increment && !isHistoryAdd) {
				...
				if (!chatOpen && ctx.duration > 0) {
					const text = $("<p>")
						...
						.append($("<div>").html(ctx.text).contents());
					text.each((i, e) =>
						html10n.translateElement(html10n.translations, e),
					);
					$.gritter.add({
						text,
						sticky: ctx.sticky,
