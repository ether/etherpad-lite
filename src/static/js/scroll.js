...
function Scroll(outerWin) {
	// scroll settings
	this.scrollSettings =
		parent.parent.clientVars.scrollWhenFocusLineIsOutOfViewport;
	...
	function (rep, isScrollableEvent, innerHeight) {
		...
		if (shouldScrollWhenCaretIsAtBottomOfViewport) {
			...
			if (
				...
				this._isCaretAtTheBottomOfViewport(rep)
			) {
...
Scroll.prototype.scrollWhenPressArrowKeys = function (
	arrowUp,
	rep,
	innerHeight,
) {
	...
	if (this._arrowUpWasPressedInTheFirstLineOfTheViewport(arrowUp, rep)) {
		const pixelsToScroll =
			this._getPixelsToScrollWhenUserPressesArrowUp(innerHeight);
...
Scroll.prototype._isCaretAtTheBottomOfViewport = function (rep) {
	...
	const firstLineVisibleAfterCaretLine = caretPosition.getNextVisibleLine(
		lineAfterCaretLine,
		rep,
	);
	...
	if (
		caretLineIsPartiallyVisibleOnViewport ||
		lineAfterCaretLineIsPartiallyVisibleOnViewport
	) {
		...
		const nextLineBottom = caretPosition.getBottomOfNextBrowserLine(
			caretLinePosition,
			rep,
		);
...
Scroll.prototype._isLinePartiallyVisibleOnViewport = function (
	lineNumber,
	rep,
) {
	...
	return (
		(topOfLineIsAboveOfViewportBottom &&
			bottomOfLineIsOnOrBelowOfViewportBottom) ||
...
Scroll.prototype._getViewPortTopBottom = function () {
	...
	const viewportExtraSpacesAndPosition =
		this._getEditorPositionTop() +
		this._getPaddingTopAddedWhenPageViewIsEnable();
	return {
		top: theTop,
...
Scroll.prototype._getEditorPositionTop = () => {
...
Scroll.prototype._getPaddingTopAddedWhenPageViewIsEnable = function () {
	...
	const aceOuterPaddingTop = Number.parseInt($(aceOuter).css("padding-top"));
...
Scroll.prototype._isCaretAtTheTopOfViewport = function (rep) {
	...
	const firstLineVisibleBeforeCaretLine = caretPosition.getPreviousVisibleLine(
		linePrevCaretLine,
		rep,
	);
	const caretLineIsPartiallyVisibleOnViewport =
	...
	const lineBeforeCaretLineIsPartiallyVisibleOnViewport =
		this._isLinePartiallyVisibleOnViewport(
			firstLineVisibleBeforeCaretLine,
			rep,
		);
	if (
		caretLineIsPartiallyVisibleOnViewport ||
		lineBeforeCaretLineIsPartiallyVisibleOnViewport
	) {
		...
		const caretLineIsAboveViewportBottom =
			caretLinePosition.top < viewportBottom;
		...
		if (caretLineIsInsideOfViewport) {
			const prevLineTop = caretPosition.getPositionTopOfPreviousBrowserLine(
				caretLinePosition,
				rep,
			);
...
Scroll.prototype._getPixelsRelativeToPercentageOfViewport = function (
	innerHeight,
	aboveOfViewport,
) {
	let pixels = 0;
	const scrollPercentageRelativeToViewport =
		this._getPercentageToScroll(aboveOfViewport);
	if (
		scrollPercentageRelativeToViewport > 0 &&
		scrollPercentageRelativeToViewport <= 1
	) {
		pixels = Number.parseInt(innerHeight * scrollPercentageRelativeToViewport);
	}
	return pixels;
};
...
Scroll.prototype._getPixelsToScrollWhenUserPressesArrowUp = function (
	...
	let pixels = 0;
	const percentageToScrollUp =
		this.scrollSettings.percentageToScrollWhenUserPressesArrowUp;
	if (percentageToScrollUp > 0 && percentageToScrollUp <= 1) {
		pixels = Number.parseInt(innerHeight * percentageToScrollUp);
	}
	return pixels;
};
...
Scroll.prototype._scrollYPage = function (pixelsToScroll) {
	...
	if (durationOfAnimationToShowFocusline) {
		this._scrollYPageWithAnimation(
			pixelsToScroll,
...
Scroll.prototype._scrollYPageWithAnimation = function (
	pixelsToScroll,
	durationOfAnimationToShowFocusline,
) {
...
Scroll.prototype.scrollNodeVerticallyIntoView = function (rep, innerHeight) {
	...
	if (linePosition) {
		...
		const distanceOfBottomOfViewport =
			viewport.bottom - linePosition.bottom - linePosition.height;
...
Scroll.prototype._partOfRepLineIsOutOfViewport = function (
	viewportPosition,
	rep,
) {
...
Scroll.prototype._getLineEntryTopBottom = (entry, destObj) => {
...
Scroll.prototype._arrowUpWasPressedInTheFirstLineOfTheViewport = function (
	arrowUp,
	rep,
) {
	const percentageScrollArrowUp =
		this.scrollSettings.percentageToScrollWhenUserPressesArrowUp;
	return (
		percentageScrollArrowUp && arrowUp && this._isCaretAtTheTopOfViewport(rep)
	);
};
...
Scroll.prototype.getVisibleLineRange = function (rep) {
	...
	const obj = {};
	const start = rep.lines.search(
		(e) => this._getLineEntryTopBottom(e, obj).bottom > viewport.top,
	);
	...
	let end = rep.lines.search(
		(e) => this._getLineEntryTopBottom(e, obj).top >= viewport.bottom,
	);
...
Scroll.prototype.getVisibleCharRange = function (rep) {
	...
	return [
		rep.lines.offsetOfIndex(lineRange[0]),
