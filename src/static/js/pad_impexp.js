...
const padimpexp = (() => {
	...
	const importErrorMessage = (status) => {
		...
		const msg = html10n.get(
			`pad.impexp.${known.indexOf(status) !== -1 ? status : "copypaste"}`,
		);

		const showError = (fade) => {
			const popup = $("#importmessagefail")
				.empty()
				.append(
					$("<strong>")
						.css("color", "red")
	...
	const self = {
		init: (_pad) => {
			...
			const padRootPath =
				/.*\/p\/[^/]+/.exec(document.location.pathname) || clientVars.padId;
