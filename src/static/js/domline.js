...
domline.createDomLine = (nonEmpty, doesWrap, optBrowser, optDocument) => {
	...
	result.appendSpan = (txt, cls) => {
			...
			if (listType) {
				...
				if (listType) {
					if (listType.indexOf("number") < 0) {
						preHtml += `<ul class="list-${Security.escapeHTMLAttribute(
							listType,
						)}"><li>`;
						postHtml = `</li></ul>${postHtml}`;
					} else {
						if (start) {
							// is it a start of a list with more than one item in?
							if (Number.parseInt(start[1]) === 1) {
								// if its the first one at this level?
								// Add start class to DIV node
								lineClass = `${lineClass} ` + `list-start-${listType}`;
							}
							preHtml += `<ol start=${
								start[1]
							} class="list-${Security.escapeHTMLAttribute(listType)}"><li>`;
						} else {
							// Handles pasted contents into existing lists
							preHtml += `<ol class="list-${Security.escapeHTMLAttribute(
								listType,
							)}"><li>`;
						}
						postHtml += "</li></ol>";
		...
		if (!txt && cls) {
			...
			if (href) {
				const urn_schemes = /^(about|geo|mailto|tel):/;
