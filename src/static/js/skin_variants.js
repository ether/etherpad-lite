...
if (window.location.hash.toLowerCase() === "#skinvariantsbuilder") {
	...
	const updateSkinVariantsClasses = () => {
		...
		$("#skin-variants-result").val(
			`"skinVariants": "${newClasses.join(" ")}",`,
		);
	};
	...
	const updateCheckboxFromSkinClasses = () => {
		$("html")
			...
			.split(" ")
			.forEach((classItem) => {
				const container = classItem.substring(
					classItem.lastIndexOf("-") + 1,
					classItem.length,
				);
		...
		$("#skin-variant-full-width").prop(
			"checked",
			$("html").hasClass("full-width-editor"),
		);
	};
