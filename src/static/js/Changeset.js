 ...
 */
exports.parseNum = (str) => Number.parseInt(str, 36);
...
exports.newOp = (optOpcode) => {
	padutils.warnDeprecated(
		"Changeset.newOp() is deprecated; use the Changeset.Op class instead",
	);
...
const opsFromText = function* (opcode, text, attribs = "", pool = null) {
	...
	op.attribs =
		typeof attribs === "string"
			? attribs
			: new AttributeMap(pool).update(attribs || [], opcode === "+").toString();
...
exports.smartOpAssembler = () => {
	...
	const appendOpWithText = (opcode, text, attribs, pool) => {
		padutils.warnDeprecated(
			"Changeset.smartOpAssembler().appendOpWithText() is deprecated; " +
...
class TextLinesMutator {
		...
		if (this._isCurLineInSplice()) {
			if (this._curCol === 0) {
				...
				this._curSplice[sline] =
					this._curSplice[sline].substring(0, this._curCol) +
					this._linesGet(this._curSplice[0] + this._curSplice[1]);
				this._curSplice[1] += 1;
			}
		} else {
	...
	remove(N, L) {
		...
		const removed = this._curSplice[sline].substring(
			this._curCol,
			this._curCol + N,
		);
		this._curSplice[sline] =
...
exports.applyToAttribution = (cs, astr, pool) => {
	const unpacked = exports.unpack(cs);
	return applyZip(astr, unpacked.ops, (op1, op2) =>
		slicerZipperFunc(op1, op2, pool),
	);
};
...
exports.mutateAttributionLines = (cs, lines, pool) => {
	...
	const outputMutOp = (op) => {
		...
		assert(
			op.lines === 1,
			`Can't have op.lines of ${op.lines} in attribution lines`,
		);
		// ship it to the mut
...
exports.filterAttribNumbers = (cs, filter) =>
	exports.mapAttribNumbers(cs, filter);
	...
	const self = {
		...
		keep: (N, L, attribs, pool) => {
			...
			o.attribs =
				typeof attribs === "string"
					? attribs
					: new AttributeMap(pool).update(attribs || []).toString();
			o.chars = N;
			o.lines = L || 0;
			assem.append(o);
			return self;
...
exports.follow = (cs1, cs2, reverseInsertOrder, pool) => {
	...
	assert(
		len1 === len2,
		"mismatched follow - cannot transform cs1 on top of cs2",
	);
	...
	const hasInsertFirst = exports.attributeTester(
		["insertorder", "first"],
		pool,
	);
