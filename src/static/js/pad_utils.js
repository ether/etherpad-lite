...
const urlRegex = (() => {
	...
	const urlChar = `[-:@_.,~%+/?=&#!;()\\[\\]$'*${wordCharRegex.source.slice(
		1,
		-1,
	)}]`;
	...
	const withoutAuth = `(?:${["about", "geo", "mailto", "tel"].join("|")}):`;
	return new RegExp(
...
const base64url =
	/^(?=(?:.{4})*$)[A-Za-z0-9_-]*(?:[AQgw]==|[AEIMQUYcgkosw048]=)?$/;
...
const padutils = {
	...
	uniqueId: () => {
		...
		const encodeNum = (n, width) =>
			(Array(width + 1).join("0") + Number(n).toString(35)).slice(-width);
		return [
			pad.getClientIp(),
	...
	encodeUserId: (userId) =>
		userId.replace(/[^a-y0-9]/g, (c) => {
			if (c === ".") return "-";
			return `z${c.charCodeAt(0)}z`;
		}),
	decodeUserId: (encodedUserId) =>
		encodedUserId.replace(/[a-y0-9]+|-|z.+?z/g, (cc) => {
			if (cc === "-") {
				return ".";
			...
			if (!msgAlreadyVisible) {
				...
				const errorMsg = [
					...
					$("<div>")
						...
						.css("font-size", ".8em")
						.css("margin-top", "1em")
						.append($("<b>").addClass("error-msg").text(msg))
						.append($("<br>"))
						.append(txt(`at ${url} at line ${linenumber}`))
						.append($("<br>"))
						.append(txt(`ErrorId: ${errorId}`))
						.append($("<br>"))
						.append(txt(type))
						.append($("<br>"))
						.append(txt(`URL: ${window.location.href}`))
						.append($("<br>"))
						.append(txt(`UserAgent: ${navigator.userAgent}`))
						.append($("<br>")),
				];
