...
const makeContentCollector = (
	...
	const _produceLineAttributesMarker = (state) => {
		...
		const attribs = new AttributeMap(apool)
			...
			.update(
				Object.entries(state.lineAttributes).map(([k, v]) => [k, v || ""]),
				true,
			);
	...
	cc.startNewLine = (state) => {
		if (state) {
			const atBeginningOfLine =
				lines.textOfLine(lines.length() - 1).length === 0;
	...
	cc.collectContent = function (node, state) {
		...
		if (node.nodeType === node.TEXT_NODE) {
			...
			const [hookTxt] = hooks
				.callAll("collectContentLineText", context)
				.filter((s) => typeof s === "string");
			let txt =
				context.text === node.nodeValue && hookTxt != null
					? hookTxt
					: context.text;
			...
			while (txt.length > 0) {
				...
				if (
					atBeginningOfLine &&
					Object.keys(state.lineAttributes).length !== 0
				) {
					_produceLineAttributesMarker(state);
				}
					...
					if (tname === "ul" || tname === "ol") {
						...
						if (rr && rr[1]) {
							...
							if (tname === "ul") {
								...
								if (
									(type && type.match("indent")) ||
									(cls && cls.match("indent"))
								) {
									type = "indent";
								} else {
									type = "bullet";
							...
							type += String(
								Math.min(_MAX_LIST_LEVEL, (state.listNesting || 0) + 1),
							);
						}
		...
		if (isBlock) {
			if (lines.length() - 1 === startLine) {
				...
				if (
					(state.lineAttributes && !state.lineAttributes.list) ||
		...
		if (unsupportedElements && unsupportedElements.size) {
			console.warn(
				"Ignoring unsupported elements (you might want to install a plugin): " +
			...
			for (let i = lineStrings.length - 1; i >= 0; i--) {
				...
				if (oldString.length > lineLimit + buffer) {
					...
					while (oldString.length > lineLimit) {
						...
						newAttribStrings.push(
							Changeset.subattribution(oldAttribString, 0, lengthToTake),
						);
						oldAttribString = Changeset.subattribution(
							oldAttribString,
							lengthToTake,
						);
					}
