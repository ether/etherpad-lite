...
const undoModule = (() => {
	const stack = (() => {
		...
		const pushExternalChange = (cs) => {
			...
			if (stackElements[idx].elementType === EXTERNAL_CHANGE) {
				stackElements[idx].changeset = Changeset.compose(
		...
		const _exposeEvent = (nthFromTop) => {
			...
			while (
				idx > targetIndex ||
				stackElements[idx].elementType === EXTERNAL_CHANGE
			) {
				if (stackElements[idx].elementType === EXTERNAL_CHANGE) {
					...
					if (un.backset) {
						...
						if (typeof un.selStart === "number") {
							const newSel = Changeset.characterRangeFollow(
								excs,
								un.selStart,
								un.selEnd,
							);
					...
					if (
						idx >= 2 &&
						stackElements[idx - 2].elementType === EXTERNAL_CHANGE
					) {
						ex.changeset = Changeset.compose(
							stackElements[idx - 2].changeset,
							ex.changeset,
							getAPool(),
						);
	...
	const _opcodeOccurrences = (cs, opcode) =>
		_charOccurrences(Changeset.unpack(cs).ops, opcode);
	...
	const performUndo = (eventFunc) => {
		if (undoPtr < stack.numEvents() - 1) {
			...
			const undoEvent = eventFunc(
				backsetEvent.backset,
				_getSelectionInfo(selectionEvent),
			);
