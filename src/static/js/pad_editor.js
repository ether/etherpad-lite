...
const padeditor = (() => {
	...
	const self = {
		...
		init: async (initialViewOptions, _pad) => {
			...
			const $outerdoc = $('iframe[name="ace_outer"]')
				.contents()
				.find("#outerdocbody");
		...
		initViewOptions: () => {
			...
			padutils.bindCheckboxChange($("#options-rtlcheck"), () => {
				pad.changeViewOption(
					"rtlIsTrue",
					padutils.getCheckbox($("#options-rtlcheck")),
				);
			});
			html10n.bind("localized", () => {
				pad.changeViewOption("rtlIsTrue", "rtl" === html10n.getDirection());
				padutils.setCheckbox(
					$("#options-rtlcheck"),
					"rtl" === html10n.getDirection(),
				);
			});
	...
	if (lineNumber) {
		if (lineNumber[0] === "L") {
			const $outerdoc = $('iframe[name="ace_outer"]')
				.contents()
				.find("#outerdocbody");
			const lineNumberInt = Number.parseInt(lineNumber.substr(1));
			if (lineNumberInt) {
				const $inner = $('iframe[name="ace_outer"]')
					.contents()
					.find("iframe")
					.contents()
					.find("#innerdocbody");
				...
				if (line.length !== 0) {
					...
					offsetTop += Number.parseInt(
						$outerdoc.css("padding-top").replace("px", ""),
					);
					const hasMobileLayout = $("body").hasClass("mobile-layout");
					if (!hasMobileLayout) {
						offsetTop += Number.parseInt(
							$inner.css("padding-top").replace("px", ""),
						);
					}
					const $outerdocHTML = $('iframe[name="ace_outer"]')
						.contents()
						.find("#outerdocbody")
						.parent();
