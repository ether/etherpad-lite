...
exports.getPositionTopOfPreviousBrowserLine = (caretLinePosition, rep) => {
	...
	const isCaretLineFirstBrowserLine = caretLineIsFirstBrowserLine(
		caretLinePosition.top,
		rep,
	);
	...
	if (isCaretLineFirstBrowserLine) {
		...
		const firstLineVisibleBeforeCaretLine = getPreviousVisibleLine(
			lineBeforeCaretLine,
			rep,
		);
		const linePosition = getDimensionOfLastBrowserLineOfRepLine(
...
const caretLineIsFirstBrowserLine = (caretLineTop, rep) => {
	...
	const positionOfFirstRootNode = getPositionOfRepLineAtOffset(
		firstRootNode,
		1,
	);
...
const getDimensionOfLastBrowserLineOfRepLine = (line, rep) => {
	...
	const lastRootChildNodePosition = getPositionOfRepLineAtOffset(
		lastRootChildNode.node,
		lastRootChildNode.length,
	);
...
exports.getBottomOfNextBrowserLine = (caretLinePosition, rep) => {
	...
	const isCaretLineLastBrowserLine = caretLineIsLastBrowserLineOfRepLine(
		caretLinePosition.top,
		rep,
	);
	...
	if (isCaretLineLastBrowserLine) {
		...
		const firstNextLineVisibleAfterCaretLine = getNextVisibleLine(
			nextLineAfterCaretLine,
			rep,
		);
		const linePosition = getDimensionOfFirstBrowserLineOfRepLine(
			firstNextLineVisibleAfterCaretLine,
			rep,
		);
...
const isLineVisible = (line, rep) =>
	rep.lines.atIndex(line).lineNode.offsetHeight > 0;
...
const getDimensionOfFirstBrowserLineOfRepLine = (line, rep) => {
	...
	const firstRootChildNodePosition = getPositionOfRepLineAtOffset(
		firstRootChildNode,
		1,
	);
