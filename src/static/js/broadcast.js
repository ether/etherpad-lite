...
const loadBroadcastJS = (
	...
	const padContents = {
		...
		currentLines: Changeset.splitTextLines(
		...
		splice(start, numRemoved, ...newLines) {
			...
			for (
				let i = start;
				i < start + numRemoved && i < this.currentDivs.length;
				i++
			) {
		...
		getActiveAuthors() {
			...
			for (const aline of this.alines) {
				for (const op of Changeset.deserializeOps(aline)) {
					for (const [k, v] of attributes.attribsFromString(
						op.attribs,
						this.apool,
					)) {
	...
	const applyChangeset = (
		...
		if (
			$("#options-followContents").is(":checked") ||
			...
			const goToLineNumber = (lineNumber) => {
				...
				const line = $("#innerdocbody").find(
					`div:nth-child(${lineNumber + 1})`,
				);
	...
	const loadedNewChangeset = (
		...
		const broadcasting =
			BroadcastSlider.getSliderPosition() === revisionInfo.latest;
   ...
   */
	...
	const goToRevision = (newRevision) => {
		...
		const path = window.revisionInfo.getPath(
			padContents.currentRevision,
			newRevision,
		);
	...
	changesetLoader = {
		...
		loadFromQueue() {
			const requestQueue =
				this.requestQueue1.length > 0
					? this.requestQueue1
					: this.requestQueue2.length > 0
						? this.requestQueue2
						: this.requestQueue3.length > 0
							? this.requestQueue3
							: null;
		...
		handleResponse: (data, start, granularity, callback) => {
			...
			for (let i = 0; i < data.forwardsChangesets.length; i++) {
				...
				const forwardcs = Changeset.moveOpsToNewPool(
					data.forwardsChangesets[i],
					pool,
					padContents.apool,
				);
				const backwardcs = Changeset.moveOpsToNewPool(
					data.backwardsChangesets[i],
					pool,
					padContents.apool,
				);
				window.revisionInfo.addChangeset(
					astart,
					aend,
					forwardcs,
					backwardcs,
			...
			if (callback)
				callback(
					start - 1,
					start + data.forwardsChangesets.length * granularity - 1,
				);
		},
			...
			if (obj.type === "COLLABROOM") {
				...
				if (obj.type === "NEW_CHANGES") {
					const changeset = Changeset.moveOpsToNewPool(
						obj.changeset,
						new AttribPool().fromJsonable(obj.apool),
						padContents.apool,
					);

					let changesetBack = Changeset.inverse(
						obj.changeset,
						padContents.currentLines,
						padContents.alines,
						padContents.apool,
					);
				...
				hooks.callAll(`handleClientTimesliderMessage_${obj.type}`, {
					payload: obj,
				});
	...
	fireWhenAllScriptsAreLoaded.push(() => {
		...
		for (let i = 0; i < padContents.currentLines.length; i++) {
			const div = padContents.lineToElement(
				padContents.currentLines[i],
	...
	const dynamicCSS = makeCSSManager(
		document.querySelector('style[title="dynamicsyntax"]').sheet,
	);
	const authorData = {};
	...
	const receiveAuthorData = (newAuthorData) => {
		for (const [author, data] of Object.entries(newAuthorData)) {
			const bgcolor =
				typeof data.colorId === "number"
					? clientVars.colorPalette[data.colorId]
					: data.colorId;
			if (bgcolor) {
				const selector = dynamicCSS.selectorStyle(
					`.${linestylefilter.getAuthorClassName(author)}`,
				);
				selector.backgroundColor = bgcolor;
				selector.color =
					colorutils.luminosity(colorutils.css2triple(bgcolor)) < 0.5
						? "#ffffff"
						: "#000000"; // see ace2_inner.js for the other part
			}
