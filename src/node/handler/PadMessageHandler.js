...
exports.setSocketIO = function(socket_io, socket_io_emitter)
{
  ...
  //Initalize sessioninfos_update_from_other_instances() so as to receive sessioninfo updates
  //from other instances in the cluster
...
exports.handleDisconnect = function(client)
{
  ...
  if(session && session.author)
  {
    ...
    authorManager.getAuthorColorId(session.author, function(err, color)
    {
      ...
      //Go through all users that are still on the pad, and send them the USER_LEAVE message
...
exports.handleCustomObjectMessage = function (msg, sessionID, cb) {
  if(msg.data.type === "CUSTOM"){
    if(sessionID){ // If a sessionID is targeted then send directly to this sessionID
      ...
      socketio_broadcast(msg.data.payload.padId, null, msg); // broadcast to all clients on this pad
    }
  }
  cb(null, {});
}
...
function handleSuggestUserName(client, message)
{
  ...
  async.series([
    function(callback){
      ...
      // an array containing all connected socket ids in room 'padId' in whole cluster
...
{
  //skip this step if no one is on this pad
  var roomClients = [];

  async.series([
    function(callback){
      socketio.sockets.in(pad.id).clients( function(err, clients){
        // an array containing all connected socket ids in room 'pad.id'
        roomClientIds = clients.slice();
        callback(null);
      });
    },
            ...
            async.waterfall([
              ...
              function(revision, callback)
              {
                ...
                if(sessioninfos[sid]){
                  ...
                  // Update sessioninfos on other instances in cluster
    ...
    function(callback){
      async.forEach(roomClientIds, function(roomClientId, callback) {
        ...
        if(sinfo && sinfo.author == currentSession.author) {
          ...
          update_sessioninfos_on_other_instances(sessioninfos[roomClientId], roomClientId);
          client.leave(padId);
...
function handleClientReady(client, message)
{
  ...
  async.series([
    ...
    function(callback)
    {
      ...
      // Update sessioninfo on other instances in the cluster
      update_sessioninfos_on_other_instances(sessioninfos[client.id], client.id);
      ...
      socketio_broadcast(padIds.padId, client.id, messageToTheOtherUsers);
...
/**
 * Updates sessioninfo of client on other instances. This function sends sessioninfo updates to all
 * etherpad instances whenever there is an update in sessioninfo
 */
...
/**
 * This function mimics client.broadcast.to(padIds.padId).json.send(messageToTheOtherUsers). It
 * uses socket.io-redis for sending events to clients connected on other instances. It basically
 * broadcasts events to socket.io clients in whole cluster
 */
function socketio_broadcast(roomId, client, message) {
  /**
   * roomId: broadcast to all clients connected in room 'roomId'
   * client: broadcast to all clients except this 'client'
   * message: message to be broadcasted across cluster
   */
  var roomClientIds = [];
  async.series([
    function(callback)
    {
      socketio.sockets.in(roomId).clients( function(err, clients){
        // an array containing all connected socket ids across all instances in room 'roomId'
        roomClientIds = clients.slice();
        callback(null);
      });
    },
...
exports.padUsers = function (padID, callback) {
  ...
  async.series([
    ...
    {
      async.forEach(roomClientIds, function(roomClientId, callback) {
        var s = sessioninfos[roomClientId];
        if(s) {
