...
import type { APool, AText, PadAuthor, PadType } from "../types/PadType";
import type {
	ChangesetRequest,
	PadUserInfo,
	SocketClientRequest,
...
const addContextToError = (err: any, pfx: string) => {
	...
	if (Error.captureStackTrace)
		Error.captureStackTrace(newErr, addContextToError);
...
class Channels {
	...
	async enqueue(ch: any, task: any): Promise<any> {
		const p = (this._promiseChains.get(ch) || Promise.resolve()).then(() =>
			this._exec(ch, task),
		);
		const pc = p
...
exports.handleDisconnect = async (socket: any) => {
	...
	const {
		session: { user } = {},
	} = socket.client.request as SocketClientRequest;
...
exports.handleMessage = async (socket: any, message: typeof ChatMessage) => {
	...
	if (env === "production") {
		try {
			...
			messageLogger.warn(
				`Rate limited IP ${socket.request.ip}. To reduce the amount of rate ` +
	...
	if (message.type === "CLIENT_READY") {
		...
		if (!(await padManager.doesPadExist(thisSession.auth.padID))) {
			thisSession.auth.padID = await padManager.sanitizePadId(
				thisSession.auth.padID,
			);
		}
		...
		thisSession.readonly =
			padIds.readonly ||
			!webaccess.userCanModify(thisSession.auth.padID, socket.client.request);
	}
	...
	Object.defineProperty(message, "padId", {
		get: () => {
			throw new Error(
				"message.padId must not be accessed (for security reasons)",
			);
		},
	});
	...
	if (!auth) {
		const ip = settings.disableIPlogging
			? "ANONYMOUS"
			: socket.request.ip || "<unknown>";
	...
	for (const res of await hooks.aCallAll("handleMessageSecurity", context)) {
		switch (res) {
			...
			default:
				messageLogger.warn(
					"Ignoring unsupported return value from handleMessageSecurity hook function:",
					res,
				);
		}
	}
	...
	if (
		(await hooks.aCallAll("handleMessage", context)).some(
 ...
 */
exports.handleCustomObjectMessage = (
	msg: typeof ChatMessage,
	sessionID: string,
) => {
 ...
 */
exports.sendChatMessageToPadClients = async (
	mt: typeof ChatMessage | number,
	puId: string,
	text: string | null = null,
	padId: string | null = null,
) => {
	const message =
		mt instanceof ChatMessage ? mt : new ChatMessage(text, puId, mt);
...
const handleGetChatMessages = async (
	...
	if (!Number.isInteger(start))
		throw new Error(`missing or invalid start: ${start}`);
	if (!Number.isInteger(end)) throw new Error(`missing or invalid end: ${end}`);
	const count = end - start;
	if (count < 0 || count > 100)
		throw new Error(`invalid number of messages: ${count}`);
	const { padId, author: authorId } = sessioninfos[socket.id];
	const pad = await padManager.getPad(padId, null, authorId);
...
const handleUserChanges = async (socket: any, message: typeof ChatMessage) => {
	...
	try {
		...
		const pad = await padManager.getPad(
			thisSession.padId,
			null,
			thisSession.author,
		);
		...
		for (const op of Changeset.deserializeOps(
			Changeset.unpack(changeset).ops,
		)) {
			...
			const opAuthorId = AttributeMap.fromString(op.attribs, wireApool).get(
				"author",
			);
			if (opAuthorId && opAuthorId !== thisSession.author) {
				throw new Error(
					`Author ${thisSession.author} tried to submit changes as author ` +
		...
		let rebasedChangeset = Changeset.moveOpsToNewPool(
			changeset,
			wireApool,
			pad.pool,
		);
		...
		while (r < pad.getHeadRevisionNumber()) {
			...
			const {
				changeset: c,
				meta: { author: authorId },
			} = await pad.getRevision(r);
			if (changeset === c && thisSession.author === authorId) {
				// Assume this is a retransmission of an already applied changeset.
				rebasedChangeset = Changeset.identity(
					Changeset.unpack(changeset).oldLen,
				);
			}
		...
		if (Changeset.oldLen(rebasedChangeset) !== prevText.length) {
			throw new Error(
				`Can't apply changeset ${rebasedChangeset} with oldLen ` +
					`${Changeset.oldLen(rebasedChangeset)} to document of length ${
						prevText.length
					}`,
			);
		}

		const newRev = await pad.appendRevision(
			rebasedChangeset,
			thisSession.author,
		);
		...
		if (pad.text().lastIndexOf("\n") !== pad.text().length - 1) {
			const nlChangeset = Changeset.makeSplice(
				pad.text(),
				pad.text().length - 1,
				0,
				"\n",
		...
		messageLogger.warn(
			`Failed to apply USER_CHANGES from author ${thisSession.author} ` +
				`(socket ${socket.id}) on pad ${thisSession.padId}: ${
					err.stack || err
				}`,
		);
	} finally {
		...
		roomSockets.map(async (socket) => {
			...
			while (sessioninfo.rev < pad.getHeadRevisionNumber()) {
				...
				try {
					...
					messageLogger.error(
						`Failed to notify user of new revision: ${err.stack || err}`,
					);
					return;
				}
...
const _correctMarkersInPad = (atext: AText, apool: APool) => {
	...
	for (const op of Changeset.deserializeOps(atext.attribs)) {
		...
		const hasMarker = AttributeManager.lineAttributes.some((a: string) =>
			attribs.has(a),
		);
		if (hasMarker) {
...
const handleClientReady = async (socket: any, message: typeof ChatMessage) => {
	...
	if (authorColorId && !/^#(?:[0-9A-F]{3}){1,2}$/i.test(authorColorId)) {
		messageLogger.warn(
			`Ignoring invalid colorId in CLIENT_READY message: ${authorColorId}`,
		);
		authorColorId = null;
	}
	await Promise.all([
		...
		authorColorId &&
			authorManager.setAuthorColorId(sessionInfo.author, authorColorId),
	]);
	({ colorId: authorColorId, name: authorName } = await authorManager.getAuthor(
		sessionInfo.author,
	));
	...
	await Promise.all(
		authors.map(async (authorId: string) => {
			...
			if (!author) {
				messageLogger.error(
					`There is no author for authorId: ${authorId}. ` +
	...
	if (sessionInfo !== sessioninfos[socket.id])
		throw new Error("client disconnected");
	...
	if (message.reconnect) {
		...
		await Promise.all(
			revisionsNeeded.map(async (revNum) => {
		...
		for (const r of revisionsNeeded) {
			const forWire = Changeset.prepareForWire(
				changesets[r].changeset,
				pad.pool,
			);
			const wireMsg = {
				type: "COLLABROOM",
			...
			scrollWhenFocusLineIsOutOfViewport: {
				percentage: {
					editionAboveViewport:
						settings.scrollWhenFocusLineIsOutOfViewport.percentage
							.editionAboveViewport,
				...
				scrollWhenCaretIsInTheLastLineOfViewport:
					settings.scrollWhenFocusLineIsOutOfViewport
						.scrollWhenCaretIsInTheLastLineOfViewport,
		...
		const messages = await hooks.aCallAll("clientVars", {
			clientVars,
			pad,
			socket,
		});
	...
	await Promise.all(
		_getRoomSockets(pad.id).map(async (roomSocket) => {
			...
			const authorInfo =
				historicalAuthorData[authorId] ||
				(await authorManager.getAuthor(authorId));
			if (authorInfo == null) {
				messageLogger.error(
 ...
 */
const handleChangesetRequest = async (
	socket: any,
	{ data: { granularity, start, requestID } }: ChangesetRequest,
) => {
	if (granularity == null) throw new Error("missing granularity");
	if (!Number.isInteger(granularity))
		throw new Error("granularity is not an integer");
	...
	if (start > headRev) start = headRev;
	const data: MapArrayType<any> = await getChangesetInfo(
		pad,
		start,
		end,
		granularity,
...
const getChangesetInfo = async (
	...
	for (
		let compositeStart = startNum;
		compositeStart < endNum;
		...
		const backwards = Changeset.inverse(
			forwards,
			lines.textlines,
			lines.alines,
			pad.apool(),
		...
		const backwards2 = Changeset.moveOpsToNewPool(
			backwards,
			pad.apool(),
			apool,
		);
		...
		const t1 =
			compositeStart === 0 ? revisionDate[0] : revisionDate[compositeStart - 1];
 ...
 */
const composePadChangesets = async (
	pad: PadType,
	startNum: number,
	endNum: number,
) => {
	...
	await Promise.all(
		changesetsNeeded.map((revNum) =>
			pad
				.getRevisionChangeset(revNum)
...
exports.padUsers = async (padID: string) => {
	...
	await Promise.all(
		_getRoomSockets(padID).map(async (roomSocket) => {
