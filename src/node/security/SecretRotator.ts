...
class Kdf {
	...
	async derive(params: DeriveModel, info: any) {
		throw new Error("not implemented");
	}
}

class LegacyStaticSecret extends Kdf {
	async derive(params: any, info: any) {
		return params;
	}
}
...
class Hkdf extends Kdf {
	...
	async derive(p: DeriveModel, info: any) {
		return Buffer.from(
			await crypto.hkdf(p.digest, p.secret, p.salt, info, p.keyLen),
		).toString("hex");
	}
}
...
const algorithms = [new LegacyStaticSecret(), new Hkdf("sha256", 32)];
...
export class SecretRotator {
	...
	constructor(
		...
		interval: number,
		lifetime: number,
		legacyStaticSecret: string | null = null,
	) {
		/**
		...
		if (/[*:%]/.test(dbPrefix))
			throw new Error(`dbPrefix contains an invalid char: ${dbPrefix}`);
		...
		this._logger.debug(
			`new secret rotator (interval ${interval}, lifetime: ${lifetime})`,
		);
	...
	async _publish(params: LegacyParams, id: string | null = null) {
		...
		// other instances.
		if (id == null)
			id = `${this._dbPrefix}:${(await crypto.randomBytes(32)).toString(
				"hex",
			)}`;
	...
	async _deriveSecrets(p: any, now: number) {
		...
		if (!p.interval)
			return [await algorithms[p.algId].derive(p.algParams, null)];
		...
		// other times.)
		for (let tN = Math.min(t0, tZ); tN >= tA && !expired(tN); tN -= p.interval)
			tNs.push(tN);
		...
		return await Promise.all(
			tNs.map(
				async (tN) => await algorithms[p.algId].derive(p.algParams, `${tN}`),
			),
		);
	}
	...
	async _update() {
		...
		const dbKeys: string[] =
			(await db.findKeys(`${this._dbPrefix}:*`, null)) || [];
		...
		await Promise.all(
			dbKeys.map(async (dbKey) => {
				...
				if (p.algId === 0 && p.algParams === this._legacyStaticSecret)
					legacyParams.push(p);
				...
				if (
					p.interval &&
					p.interval === this._interval &&
					p.lifetime === this._lifetime &&
					tA <= t1 &&
					p.end > now &&
					(currentParams == null || p.start > currentParams.start)
				) {
		...
		if (
			...
			now < legacyEnd + this._lifetime + this._interval &&
			!legacyParams.find(
				(p) => p.end + p.lifetime >= legacyEnd + this._lifetime,
			)
		) {
			const d = new Date(legacyEnd).toJSON();
			this._logger.debug(
				`adding legacy static secret for ${d} with lifetime ${this._lifetime}`,
			);
		...
		await Promise.all(
			allParams.map(async (p) =>
				secrets.push(...(await this._deriveSecrets(p, now))),
			),
		);
		...
		this._updateTimeout = this._t.setTimeout(
			async () => await this._update(),
			next - this._t.now(),
		);
	}
}
