...
const storage = new LRUCache<string, AdapterPayload | string[] | string>(
	options,
);
...
class MemoryAdapter implements Adapter {
	...
	destroy(id: string) {
		...
		if (grantId) {
			...
			(storage.get(grantKey) as string[])!.forEach((token) =>
	...
	find(id: string): Promise<AdapterPayload | void | undefined> {
		if (storage.has(this.key(id))) {
			return Promise.resolve<AdapterPayload>(
				storage.get(this.key(id)) as AdapterPayload,
			);
		}
