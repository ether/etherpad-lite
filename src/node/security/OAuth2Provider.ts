 ...
 */
export const expressCreateServer = async (
	hookName: string,
	...
	const oidc = new Provider(settings.sso.issuer, {
		...
		extraTokenClaims: async (ctx, token) => {
			if (token.kind === "AccessToken") {
				...
				const account = usersArray1.find(
					(user) => user.username === token.accountId,
				);
				return {
	...
	args.app.post(
		...
		async (
			...
			try {
				...
				const { prompt, jti, session, cid, params, grantId } =
				...
				switch (prompt.name) {
					case "login": {
						...
						const account = usersArray1.find(
							(user) =>
								user.username === (login as unknown as string) &&
								user.password === (password as unknown as string),
						);
						if (!account) {
		...
		async (req: Request, res: Response, next: Function) => {
			try {
				...
				switch (prompt.name) {
					...
					default:
						return res.sendFile(
							path.join(settings.root, "src", "static", "oidc", "login.html"),
						);
				}
			} catch (err) {
				return next(err);
	...
	args.app.use(
		"/views/",
		express.static(path.join(settings.root, "src", "static", "oidc"), {
