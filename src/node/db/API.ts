 ...
 * GROUP FUNCTIONS ****
 ******************** */
  listAllGroups: groupManager.listAllGroups,
  createGroup: groupManager.createGroup,
  createGroupIfNotExistsFor: groupManager.createGroupIfNotExistsFor,
  deleteGroup: groupManager.deleteGroup,
  listPads: groupManager.listPads,
 ...
 ******************** */
  ...
  createAuthor: authorManager.createAuthor,
  createAuthorIfNotExistsFor: authorManager.createAuthorIfNotExistsFor,
  getAuthorName: authorManager.getAuthorName,
  listPadsOfAuthor: authorManager.listPadsOfAuthor,
  padUsers: padMessageHandler.padUsers,
  padUsersCount: padMessageHandler.padUsersCount,
  ...
  listSessionsOfGroup: sessionManager.listSessionsOfGroup,
 ...
 *********************** */
   ...
   */
  getAttributePool: async (padID: string) => {
   ...
   */
  getRevisionChangeset: async (padID: string, rev: number) => {
   ...
   */
   getText: async (padID: string, rev: number) => {
    // try to parse the revision number
    if (rev !== undefined) {
      rev = checkValidRev(rev);
    }
   ...
   */
  setText: async (padID: string, text?: string, authorId: string = ''): Promise<void> => {
    // text is required
  ...
  /**
   ...
   @param {String} authorId the id of the author, defaulting to empty string
   */
  appendText: async (padID:string, text?: string, authorId:string = '') => {
    // text is required
    if (typeof text !== 'string') {
      throw new CustomError('text is not a string', 'apierror');
    }
   ...
   */
  getHTML: async (padID: string, rev: number): Promise<{ html: string; }> => {
    if (rev !== undefined) {
      rev = checkValidRev(rev);
    }
  ...
  /**
   ...
   @param {String} authorId the id of the author, defaulting to empty string
   */
  setHTML: async (padID: string, html:string|object, authorId = '') => {
 ...
 **************** */
   ...
   */
  getChatHistory: async (padID: string, start:number, end:number) => {
    if (start && end) {
      if (start < 0) {
   ...
   */
  appendChatMessage: async (padID: string, text: string|object, authorID: string, time: number) => {
    // text is required
    if (typeof text !== 'string') {
      throw new CustomError('text is not a string', 'apierror');
    }
  ...
  /**
   ...
   @param {String} padID the id of the pad
   */
  getRevisionsCount: async (padID: string) => {
    // get the pad
    const pad = await getPadSafe(padID, true);
  ...
  /**
   ...
   @param {String} padID the id of the pad
   */
  getSavedRevisionsCount: async (padID: string) => {
    // get the pad
    const pad = await getPadSafe(padID, true);
  ...
  /**
   ...
   @param {String} padID the id of the pad
   */
  listSavedRevisions: async (padID: string) => {
    // get the pad
    const pad = await getPadSafe(padID, true);
   ...
   */
  saveRevision: async (padID: string, rev: number) => {
   ...
   */
  getLastEdited: async (padID: string): Promise<{ lastEdited: number; }> => {
    // get the pad
    const pad = await getPadSafe(padID, true);
  ...
  /**
   ...
   @param {String} authorId the id of the author, defaulting to empty string
   */
  createPad: async (padID: string, text: string, authorId = '') => {
    if (padID) {
  ...
  /**
   ...
   @param {String} authorId the id of the author, defaulting to empty string
   */
  restoreRevision: async (padID: string, rev: number, authorId = '') => {
    // check if rev is a number
    if (rev === undefined) {
   ...
   */
  copyPad: async (sourceID: string, destinationID: string, force: boolean) => {
  ...
  /**
   ...
   @param {String} authorId the id of the author, defaulting to empty string
   */
  copyPadWithoutHistory: async (sourceID: string, destinationID: string, force:boolean, authorId = '') => {
    const pad = await getPadSafe(sourceID, true);
  ...
  /**
   ...
   @param {Boolean} force whether to overwrite the destination pad if it exists
   */
  movePad: async (sourceID: string, destinationID: string, force:boolean) => {
    const pad = await getPadSafe(sourceID, true);
    await pad.copy(destinationID, force);
    await pad.remove();
  },
  ...
  /**
   ...
   @param {String} padID the id of the pad
   */
  getReadOnlyID: async (padID: string) => {
   ...
   */
  setPublicStatus: async (padID: string, publicStatus: boolean|string) => {
    // ensure this is a group pad
  ...
  /**
   ...
   @param {String} padID the id of the pad
   */
  getPublicStatus: async (padID: string) => {
    // ensure this is a group pad
    checkGroupPad(padID, 'publicStatus');
  ...
  /**
   ...
   @param {String} padID the id of the pad
   */
  listAuthorsOfPad: async (padID: string) => {
    // get the pad
    const pad = await getPadSafe(padID, true);
  ...
  sendClientsMessage: async (padID: string, msg: string) => {
   ...
   */
  getChatHead: async (padID:string): Promise<{ chatHead: number; }> => {
    // get the pad
    const pad = await getPadSafe(padID, true);
   ...
   */
  createDiffHTML: async (padID: string, startRev: number, endRev: number) => {
