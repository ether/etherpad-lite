...
const getHTMLFromAtext = async (
	...
	await Promise.all([
		...
		hooks
			.aCallAll("exportHtmlAdditionalTags", pad)
			.then((newProps: string[]) => {
		...
		hooks
			.aCallAll("exportHtmlAdditionalTagsWithData", pad)
			.then((newProps: string[]) => {
				newProps.forEach((prop) => {
		...
		if (urls) {
			urls.forEach(
				(
					...
					assem.append(
						`<a href="${Security.escapeHTMLAttribute(
							url,
	...
	for (let i = 0; i < textLines.length; i++) {
		...
		if (line.listLevel) {
			...
			if (
				!prevLine ||
				prevLine.listLevel !== line.listLevel ||
              ...
              */
						...
						if (line.listTypeName === "number") {
							...
							if (line.start) {
								pieces.push(
									`<ol start="${Number(line.start)}" class="${
										line.listTypeName
									}">`,
								);
							} else {
			...
			if (
				...
				for (let diff = nextLevel; diff < line.listLevel; diff++) {
					openLists = openLists.filter(
						(el) => el.level !== diff && el.type !== line.listTypeName,
					);

					if (
						pieces[pieces.length - 1].indexOf("</ul") === 0 ||
...
exports.getPadHTMLDocument = async (
	padId: string,
	revNum: string,
	readOnlyId: number,
) => {
	...
	const stylesForExport: string[] = await hooks.aCallAll(
		"stylesForExport",
		padId,
	);
	...
	for (const hookHtml of await hooks.aCallAll("exportHTMLAdditionalContent", {
		padId,
	})) {
		html += hookHtml;
	}
