...
class PadDiff {
	...
	constructor(pad: PadType, fromRev: string, toRev: string) {
		...
		if (!range)
			throw new Error(
				`Invalid revision range. startRev: ${fromRev} endRev: ${toRev}`,
			);
	...
	_isClearAuthorship(changeset: any) {
		...
		if (
			clearOperator.chars !== unpacked.oldLen - 1 &&
			clearOperator.chars !== unpacked.oldLen
		) {
			return false;
		}

		const [appliedAttribute, anotherAttribute] = attributes.attribsFromString(
			clearOperator.attribs,
			this._pad.pool,
		);
	...
	async _getChangesetsInBulk(startRev: any, count: any) {
		...
		await Promise.all(
			revisions.map((rev) =>
				this._pad.getRevision(rev).then((revision) => {
	...
	async _createDiffAtext() {
		...
		for (let rev = this._fromRev + 1; rev <= this._toRev; rev += bulkSize) {
			// get the bulk
			const { changesets, authors } = await this._getChangesetsInBulk(
				rev,
				bulkSize,
			);
			...
			for (let i = 0; i < changesets.length && rev + i <= this._toRev; ++i) {
				...
				changeset = this._extendChangesetWithAuthor(
					changeset,
					authors[i],
					this._pad.pool,
				);
				...
				if (superChangeset == null) {
					...
					superChangeset = Changeset.compose(
						superChangeset,
						changeset,
						this._pad.pool,
					);
	...
	async getHtml() {
		...
		this._html = await exportHtml.getHTMLFromAtext(
			this._pad,
			atext,
			authorColors,
		);
	...
	_extendChangesetWithAuthor(changeset: any, author: any, apool: any) {
		...
		const attribs = `*${Changeset.numToString(
			authorAttrib,
		)}*${Changeset.numToString(deletedAttrib)}`;
	...
	_createDeletionChangeset(cs: any, startAText: any, apool: any) {
		...
		const alines = Changeset.splitAttributionLines(
			startAText.attribs,
			startAText.text,
		);
		...
		const consumeAttribRuns = (
			numChars: number,
			func: Function /* (len, attribs, endsLine)*/,
		) => {
		...
		for (const csOp of Changeset.deserializeOps(unpacked.ops)) {
			if (csOp.opcode === "=") {
				...
				if (csOp.attribs && textBank !== "*") {
					...
					while (textLeftToProcess.length > 0) {
						...
						if (lineBreak) {
							...
							consumeAttribRuns(
								lengthToProcess,
								(len: number, attribs: string, endsLine: string) => {
									// get the old attributes back
									const oldAttribs = undoBackToAttribs(attribs);

									builder.insert(
										processText.substr(textBankIndex, len),
										oldAttribs,
									);
									textBankIndex += len;
								},
				...
				consumeAttribRuns(
					...
					(len: number, attribs: string[], endsLine: string) => {
						builder.insert(
							textBank.substr(textBankIndex, len),
							attribs + csOp.attribs,
						);
						textBankIndex += len;
					},
				);
