...
const generateCacheKeyWithSha256 = (path: string) =>
	_crypto.createHash("sha256").update(path).digest("hex");

const generateCacheKeyWithDjb2 = (path: string) =>
	Buffer.from(djb2Hash(path)).toString("hex");
...
if (_crypto) {
	...
	console.warn(
		"No crypto support in this nodejs runtime. Djb2 (weaker) will be used.",
	);
}
...
export default class CachingMiddleware {
	...
	async _handle(req: any, res: any, next: any) {
		...
		const stats = await fsp
			.stat(`${CACHE_DIR}minified_${cacheKey}`)
			.catch(() => {});
		const modifiedSince =
		...
		const respond = () => {
			...
			if (
				supportsGzip &&
				/application\/javascript/.test(headers["content-type"])
			) {
				pathStr += ".gz";
			...
			const lastModified =
				headers["last-modified"] && new Date(headers["last-modified"]);
		...
		const expirationDate = new Date(
			((responseCache[cacheKey] || {}).headers || {}).expires,
		);
		...
		res.writeHead = (status: number, headers: Headers) => {
			...
			if (status === 200) {
				...
				res.end = async (data: number, encoding: number) => {
					await Promise.all([
						fsp
							.writeFile(`${CACHE_DIR}minified_${cacheKey}`, buffer)
							.catch(() => {}),
						util
							.promisify(zlib.gzip)(buffer)
							// @ts-ignore
							.then((content: string) =>
								fsp.writeFile(`${CACHE_DIR}minified_${cacheKey}.gz`, content),
							)
							.catch(() => {}),
					]);
				...
				oldRes.end = res.end;
				res.write = (data: number, encoding: number) => {};
				res.end = (data: number, encoding: number) => {
					respond();
				};
			} else {
