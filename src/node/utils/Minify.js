...
const threadsPool = new Threads.Pool(
	() => Threads.spawn(new Threads.Worker("./MinifyWorker")),
	2,
);
...
const requestURI = async (url, method, headers) => {
	...
	const mockRequest = {
		...
		method,
		params: {
			filename: (parsedUrl.pathname + parsedUrl.search).replace(
				/^\/static\//,
				"",
			),
		},
	...
	Promise.all(
		locations.map(async (loc) => {
			try {
				...
				logger.debug(
					`requestURI(${JSON.stringify(loc)}, ${JSON.stringify(method)}, ` +
...
const minify = async (req, res) => {
	...
	try {
		...
		logger.error(
			`sanitization of pathname "${filename}" failed: ${err.stack || err}`,
		);
		res.writeHead(404, {});
		res.end();
		return;
	...
	const newLocation =
		compatPaths[filename.replace(/^plugins\/ep_etherpad-lite\/static\//, "")];
	if (newLocation != null) {
		logger.warn(
			`request for deprecated path "${filename}", replacing with "${newLocation}"`,
		);
  ...
  */
	...
	const [, testf] =
		/^plugins\/ep_etherpad-lite\/(tests\/frontend\/.*)/.exec(filename) || [];
...
const requireDefinition = () =>
	`var require = ${RequireKernel.kernelSource};\n`;
			...
			threadsPool.queue(async ({ compressJS }) => {
				try {
					...
					if (compressResult.error) {
						console.error(
							`Error compressing JS (${filename}) using terser`,
		...
		return await new Promise((resolve) => {
			threadsPool.queue(async ({ compressCSS }) => {
				try {
					...
					console.error(
						`CleanCSS.minify() returned an error on ${filename}: ${error}`,
					);
				}
				resolve(content);
			});
...
exports.minify = (req, res, next) =>
	minify(req, res).catch((err) => next(err || new Error(err)));
