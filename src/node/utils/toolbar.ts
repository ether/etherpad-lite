...
const tagAttributes = (attributes: AttributeObj) => {
	attributes = _.reduce(
		attributes || {},
		(o: AttributeObj, val: string, name: string) => {
	...
	return ` ${_.map(
		attributes,
		(val: string, name: string) => `${name}="${_.escape(val)}"`,
	).join(" ")}`;
};
	...
	public static load(btnName: string) {
		...
		try {
			if (
				...
				button.constructor === SelectButton
			) {
				return button;
			} else {
...
module.exports = {
	...
	menu(
		...
		if (isReadOnly) {
			...
			buttons[0].indexOf("savedrevision") === -1 &&
			whichMenu === "right" &&
			page === "pad"
		) {
			/*
		...
		const groups = _.map(buttons, (group: string[]) =>
			ButtonGroup.fromArray(group).render(),
		);
