...
const loadEtherpadInformations = () => {
	if (
		...
		Date.now() - lastLoadingTime < updateInterval
	) {
		return infos;
	}

	return axios
		.get("https://static.etherpad.org/info.json", { headers: headers })
		.then(async (resp: any) => {
...
exports.check = () => {
	exports.needsUpdate((needsUpdate: boolean) => {
		if (needsUpdate) {
			console.warn(
				`Update available: Download the actual version ${infos.latestVersion}`,
			);
		}
	});
};
