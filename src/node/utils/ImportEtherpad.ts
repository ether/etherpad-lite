import type { APool } from "../types/PadType";
...
const supportedElems =
	require("../../static/js/contentcollector").supportedElems;
...
exports.setPadRaw = async (padId: string, r: string, authorId = "") => {
	...
	try {
			...
			if (prefix === "globalAuthor" && keyParts.length === 2) {
				...
				if (prefix === "pad" && keyParts.length === 2) {
					...
					if (unsupportedElements.size) {
						logger.warn(
							`(pad ${padId}) unsupported attributes (try installing a plugin): ` +
				...
				logger.debug(
					`(pad ${padId}) The record with the following key will be ignored unless an ` +
		...
		// @ts-ignore
		const readOps = new Stream(Object.entries(records)).map(([k, v]) =>
			processRecord(k, v),
		);
