...
if (os.type().indexOf("Windows") > -1) {
	exports.convertFile = async (
		...
		let stdoutBuffer = "";
		abiword.stdout.on("data", (data: string) => {
			stdoutBuffer += data.toString();
		});
		abiword.stderr.on("data", (data: string) => {
			stdoutBuffer += data.toString();
		});
		await new Promise<void>((resolve, reject) => {
			abiword.on("exit", (code: number) => {
				if (code !== 0)
					return reject(new Error(`Abiword died with exit code ${code}`));
				if (stdoutBuffer !== "") {
	...
	const spawnAbiword = () => {
		...
		abiword.stdout!.on("data", (data) => {
			...
			if (stdoutBuffer.search("AbiWord:>") !== -1) {
				const err =
					stdoutBuffer.search("OK") !== -1 ? null : new Error(stdoutBuffer);
				stdoutBuffer = "";
	...
	const queue = async.queue((task: AsyncQueueTask, callback: Function) => {
		abiword.stdin!.write(
			`convert ${task.srcFile} ${task.destFile} ${task.type}\n`,
		);
