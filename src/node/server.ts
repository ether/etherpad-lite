...
const removeSignalListener = (
	...
	logger.debug(
		`Removing ${signal} listener because it might interfere with shutdown tasks. ` +
			`Function code:\n${listener.toString()}\n` +
...
exports.start = async () => {
	...
	try {
		...
		process.on("uncaughtException", (err: ErrorCaused) => {
			...
			exports.exit(err).catch((err: ErrorCaused) => {
	...
	try {
		...
		await Promise.race([
			...
			new Promise((resolve, reject) => {
				timeout = setTimeout(
					() => reject(new Error("Timed out waiting for shutdown tasks")),
					3000,
				);
			}),
		]);
...
exports.exit = async (err: ErrorCaused | string | null = null) => {
	...
	if (err === "SIGTERM") {
		...
		logger.error(
			`Metrics at time of fatal error:\n${JSON.stringify(
				stats.toJSON(),
				null,
				2,
			)}`,
		...
		if (exitCalled) {
			logger.error(
				"Error occurred while waiting to exit. Forcing an immediate unclean exit...",
			);
			process.exit(1);
		}
	}
	...
	setTimeout(() => {
		logger.error(
			"Something that should have been cleaned up during the shutdown hook (such as " +
		...
		if (settings.dumpOnUncleanExit) {
			...
			logger.error(
				"Enable `dumpOnUncleanExit` setting to get a dump of objects preventing a " +
					"clean exit",
			);
		}
