...
function getAllLocales() {
    ...
    function extractLangs(dir) {
        ...
        fs.readdirSync(dir).forEach(function (file) {
            ...
            if ((ext == '.json') && languages.isValid(locale)) {
                if (!locales2paths[locale])
                    locales2paths[locale] = [];
    ...
    for (var pluginName in plugins)
        extractLangs(path.join(npm.root, pluginName, 'locales'));
    ...
    _.each(locales2paths, function (files, langcode) {
        ...
        files.forEach(function (file) {
            var fileContents = JSON.parse(fs.readFileSync(file, {encoding: 'utf8'}));
...
var generateLocaleIndex = function (locales) {
    // keep English strings
    var result = {};
    var defaultLang = settings.padOptions.lang;
    _.each(locales, function (val, key) {
        var embed = key === 'en' ||
            key === 'en-gb' ||
            key === defaultLang;
        var propVal = embed ? locales[key] : 'locales/' + key + '.json';
        result[key] = result[key] || propVal;
    });
